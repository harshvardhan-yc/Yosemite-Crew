...
import type { Address as FHIRAddress, Extension } from '@yosemite-crew/fhir';
...
const isGeolocationExtension = (extension?: Extension): extension is GeolocationExtension => {
  ...
  return Boolean(
    ...
    isNumber(latitude.valueDecimal) &&
    longitude?.url === 'longitude' &&
    isNumber(longitude.valueDecimal)
  );
};

const extractGeolocation = (
  extension?: Extension[]
): Pick<AddressAttributes, 'latitude' | 'longitude'> => {
