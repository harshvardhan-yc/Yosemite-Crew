...
export const MICROCHIP_IDENTIFIER_SYSTEM_URL = 'http://example.org/fhir/Identifier/microchip';
export const PASSPORT_IDENTIFIER_SYSTEM_URL = 'http://example.org/fhir/Identifier/passport';
...
const toFHIRGenderStatus = (isNeutered?: boolean): CodeableConcept | undefined => {
...
const addStringExtension = (extensions: Extension[], url: string, value?: string): void => {
  if (!value) {
    return;
  }
  extensions.push({
...
const addDecimalExtension = (extensions: Extension[], url: string, value?: number): void => {
...
const addInsuranceExtension = (extensions: Extension[], companion: Companion): void => {
  const nested: Extension[] = [
    {
      url: 'isInsured',
...
const buildExtensions = (companion: Companion): Extension[] => {
  ...
  addStringExtension(extensions, EXTENSION_AGE_WHEN_NEUTERED_URL, companion.ageWhenNeutered);
  addStringExtension(extensions, EXTENSION_COUNTRY_OF_ORIGIN_URL, companion.countryOfOrigin);
...
const parseSpecies = (species?: CodeableConcept): CompanionType | undefined => {
  if (!species) {
    return undefined;
  }
  ...
  const codingMatch = species.coding?.find(
    (coding) =>
      ...
      Object.values(COMPANION_SPECIES_MAP).some((config) => config.code === coding.code)
  );
...
const parseBreed = (breed?: CodeableConcept): Companion['breed'] | undefined => {
  const codingMatch = breed?.coding?.find((coding) => typeof coding.display === 'string');
...
const parseGenderStatus = (genderStatus?: CodeableConcept): Companion['isneutered'] => {
...
const filterExtensions = (extensions: Extension[] | undefined, url: string): Extension[] =>
...
const parseStringExtension = (
  ...
  filterExtensions(extensions, url).find((extension) => typeof extension.valueString === 'string')
    ?.valueString;
...
const parsePhysicalAttribute = (
  ...
  const extension = filterExtensions(extensions, EXTENSION_PHYSICAL_ATTRIBUTE_URL)[0];
  ...
  return Object.values(attributes).some((value) => value !== undefined && value !== null)
    ? attributes
    : undefined;
};

const parseBreedingInfo = (extensions: Extension[] | undefined): Companion['breedingInfo'] => {
  const extension = filterExtensions(extensions, EXTENSION_BREEDING_INFO_URL)[0];
  ...
  const parseParent = (url: 'sire' | 'dam'): BreedingParentDetails | undefined => {
    ...
    const dateValue = parentNested.find((ext) => ext.url === 'dateOfBirth')?.valueDate;
    ...
    return Object.values(parent).some((value) => value !== undefined && value !== null)
      ? parent
      : undefined;
  };
...
const parseMedicalRecords = (extensions: Extension[] | undefined): Companion['medicalRecords'] => {
  const records = filterExtensions(extensions, EXTENSION_MEDICAL_RECORD_URL)
    .map((recordExtension) => {
      const nested = recordExtension.extension ?? [];
      const fileUrlExtension = nested.find((ext) => ext.url === 'fileUrl');
      const fileUrl = fileUrlExtension?.valueUrl ?? fileUrlExtension?.valueUri;
      const fileName = nested.find((ext) => ext.url === 'fileName')?.valueString;
      const uploadedAtValue = nested.find((ext) => ext.url === 'uploadedAt')?.valueDateTime;
...
const parseAllergyExtension = (extensions: Extension[] | undefined): Companion['allergy'] => {
  const value = parseStringExtension(extensions, EXTENSION_ALLERGY_URL);
  return value?.trim() || undefined;
};

const parseSourceExtension = (extensions: Extension[] | undefined): SourceType | undefined => {
  const value = parseStringExtension(extensions, EXTENSION_SOURCE_URL);
  return value && SOURCE_VALUES.has(value as SourceType) ? (value as SourceType) : undefined;
};

const parsePhoto = (photos?: Patient['photo']): Companion['photoUrl'] | undefined =>
...
const parseInsuranceExtension = (
  ...
  const insuranceExtension = filterExtensions(extensions, EXTENSION_INSURANCE_URL)[0];

  const nested = insuranceExtension?.extension ?? [];
  const isInsured = nested.find((ext) => ext.url === 'isInsured')?.valueBoolean ?? false;

  const companyName = nested.find((ext) => ext.url === 'companyName')?.valueString;
  const policyNumber = nested.find((ext) => ext.url === 'policyNumber')?.valueString;
...
const findIdentifierValue = (
  ...
  identifiers?.find((identifier) => identifier.system === system)?.value || undefined;

const requireField = <T>(value: T | undefined, field: string): T => {
  if (value === undefined || value === null) {
    throw new Error(`Cannot convert FHIR Patient to Companion: missing ${field}`);
  }
  return value;
};
...
export const toFHIRCompanion = (companion: Companion): Patient => {
  ...
  return {
    ...
    id: companion.id,
    active:
      companion.status === 'active' ? true : companion.status === 'archived' ? false : undefined,
    name: companion.name
      ? [
          {
            text: companion.name,
...
export const fromFHIRCompanion = (dto: Patient): Companion => {
  ...
  return {
    ...
    isInsured,
    insurance,
    countryOfOrigin: parseStringExtension(extensions, EXTENSION_COUNTRY_OF_ORIGIN_URL),
