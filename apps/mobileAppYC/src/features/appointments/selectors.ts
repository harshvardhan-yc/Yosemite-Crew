...
import {isTerminalAppointmentStatus} from '@/features/appointments/utils/appointmentStatus';

const selectAppointments = (state: RootState) =>
  state.appointments?.items ?? [];
const selectCompanionParam = (_: RootState, companionId: string | null) =>
  companionId;
const selectBusinesses = (state: RootState) =>
  state.businesses?.businesses ?? [];
const selectServices = (state: RootState) => state.businesses?.services ?? [];
const selectBusinessCategoryParam = (
  _: RootState,
...
export const createSelectUpcomingAppointments = () => {
  ...
  return createSelector(base, appointments =>
    appointments.filter(a => !isTerminalAppointmentStatus(a.status)),
  );
};
...
export const createSelectBusinessesByCategory = () =>
  createSelector(
    [selectBusinesses, selectBusinessCategoryParam],
    (businesses, category) => {
      if (!category) {
        return businesses;
      }
...
export const selectEmployeesForBusiness =
  (businessId: string) => (state: RootState) =>
    state.businesses.employees.filter(e => e.businessId === businessId);

export const createSelectEmployeesForBusiness = () =>
  createSelector(
    [
      (state: RootState) => state.businesses.employees,
      (_: RootState, businessId: string) => businessId,
    ],
    (employees, businessId) =>
      employees.filter(e => e.businessId === businessId),
  );
...
export const selectServiceById =
  (serviceId?: string | null) => (state: RootState) =>
    serviceId
      ? (state.businesses.services.find(s => s.id === serviceId) ?? null)
      : null;
  ...
  (state: RootState) => {
    ...
    if (opts?.serviceId) {
      const match = availability.find(
        av => av.businessId === businessId && av.serviceId === opts.serviceId,
      );
      if (match) return match;
    }
    if (opts?.employeeId) {
      const match = availability.find(
        av => av.businessId === businessId && av.employeeId === opts.employeeId,
      );
      if (match) return match;
    }
...
export const selectInvoiceForAppointment =
  (appointmentId: string) => (state: RootState) =>
    state.appointments.invoices.find(
