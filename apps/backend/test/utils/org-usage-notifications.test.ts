...
describe('Org Usage Notifications Utils', () => {
  ...
  describe('Business Logic', () => {
    ...
    it('should return early if organization is not found', async () => {
      ...
      await SutModule.sendFreePlanLimitReachedEmail({ orgId: mockOrgId, usage: breachedUsage });
    ...
    it('should return early if owner user is not found', async () => {
      (OrganizationModel.findById as jest.Mock).mockReturnValue(mockChain({ name: 'Org' }));
      (UserOrganizationModel.findOne as jest.Mock).mockReturnValue(mockChain({ practitionerReference: '123' }));
    ...
    it('should return early if usage limits are not reached', async () => {
      (OrganizationModel.findById as jest.Mock).mockReturnValue(mockChain({ name: 'Org' }));
      (UserOrganizationModel.findOne as jest.Mock).mockReturnValue(mockChain({ practitionerReference: '123' }));
      (UserModel.findOne as jest.Mock).mockReturnValue(mockChainSimple({ email: 'test@test.com' }));

      await SutModule.sendFreePlanLimitReachedEmail({ orgId: mockOrgId, usage: safeUsage });

      expect(sendEmailTemplate).not.toHaveBeenCalled();
    });

    it('should send email when limits are reached', async () => {
      (OrganizationModel.findById as jest.Mock).mockReturnValue(mockChain({ name: 'Test Org', fhirId: 'fhir-123' }));
      // Test reference extraction logic (Practitioner/ID -> ID)
      (UserOrganizationModel.findOne as jest.Mock).mockReturnValue(mockChain({ practitionerReference: 'Practitioner/999' }));
      (UserModel.findOne as jest.Mock).mockReturnValue(mockChainSimple({
      ...
      expect(UserOrganizationModel.findOne).toHaveBeenCalledWith(expect.objectContaining({
      ...
      expect(UserModel.findOne).toHaveBeenCalledWith({ userId: '999' }, expect.anything());
    ...
    it('should handle optional usage values as 0 (triggering limits if limit is 0)', async () => {
        (OrganizationModel.findById as jest.Mock).mockReturnValue(mockChain({ name: 'Org' }));
        (UserOrganizationModel.findOne as jest.Mock).mockReturnValue(mockChain({ practitionerReference: '123' }));
        (UserModel.findOne as jest.Mock).mockReturnValue(mockChainSimple({ email: 'e@e.com' }));
    ...
    it('should handle organization without fhirId', async () => {
        (OrganizationModel.findById as jest.Mock).mockReturnValue(mockChain({ name: 'Org' })); // No fhirId
        (UserOrganizationModel.findOne as jest.Mock).mockReturnValue(mockChain({ practitionerReference: '123' }));
        (UserModel.findOne as jest.Mock).mockReturnValue(mockChainSimple({ email: 'e@e.com' }));
    ...
    it('should handle owner without name parts', async () => {
        (OrganizationModel.findById as jest.Mock).mockReturnValue(mockChain({ name: 'Org' }));
        (UserOrganizationModel.findOne as jest.Mock).mockReturnValue(mockChain({ practitionerReference: '123' }));
        (UserModel.findOne as jest.Mock).mockReturnValue(mockChainSimple({ email: 'e@e.com' })); // No first/last name
    ...
    it('should log error if email fails', async () => {
        ...
        expect(logger.error).toHaveBeenCalledWith("Failed to send free plan limit reached email.", err);
    });
  });
  ...
  describe('Environment Variable Fallbacks', () => {
    ...
    it('should use default support email and billing url when env vars are missing', async () => {
        ...
        const DynamicUserOrgModel = require('../../src/models/user-organization').default;
        const DynamicUserModel = require('../../src/models/user').default;
        const { sendEmailTemplate: dynamicSendEmail } = require('../../src/utils/email');

        // Setup mock on the DYNAMIC instances
        (DynamicOrgModel.findById as jest.Mock).mockReturnValue(mockChain({ name: 'Org' }));
        (DynamicUserOrgModel.findOne as jest.Mock).mockReturnValue(mockChain({ practitionerReference: '123' }));
        (DynamicUserModel.findOne as jest.Mock).mockReturnValue(mockChainSimple({ email: 'e@e.com' }));

        await DynamicSut.sendFreePlanLimitReachedEmail({ orgId: mockOrgId, usage: breachedUsage });

        expect(dynamicSendEmail).toHaveBeenCalledWith(expect.objectContaining({
            templateData: expect.objectContaining({
