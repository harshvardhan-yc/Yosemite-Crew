...
describe('Email Utils', () => {
  ...
  describe('Configuration & Client Initialization', () => {
    it('should throw error if no AWS region is configured', async () => {
      ...
      await expect(localModule.sendEmail({
        to: 'test@example.com',
        subject: 'test',
        textBody: 'test',
    ...
    it('should resolve region from AWS_SES_REGION and init client', async () => {
      ...
      await emailModule.sendEmail({ to: 'test@test.com', subject: 's', textBody: 'b', sourceEmail: 'src@test.com' });

      expect(MockSESClient).toHaveBeenCalledWith(expect.objectContaining({
        region: 'us-east-1',
    ...
    it('should resolve region from AWS_REGION if SES specific is missing', async () => {
      ...
      mockSend.mockResolvedValue({});
      await localModule.sendEmail({ to: 'test@test.com', subject: 's', textBody: 'b', sourceEmail: 'src@test.com' });
    ...
    it('should use cached client on subsequent calls', async () => {
      ...
      await emailModule.sendEmail({ to: 't1@t.com', subject: 's', textBody: 'b', sourceEmail: 'src@t.com' });
      await emailModule.sendEmail({ to: 't2@t.com', subject: 's', textBody: 'b', sourceEmail: 'src@t.com' });
  ...
  describe('sendEmail Validation', () => {
    it('should throw if "to" addresses are empty', async () => {
      await expect(emailModule.sendEmail({ to: [], subject: 's', textBody: 'b', sourceEmail: 's@s.com' }))
        .rejects.toThrow('At least one recipient is required');

      await expect(emailModule.sendEmail({ to: undefined, subject: 's', textBody: 'b' }))
        .rejects.toThrow('At least one recipient is required');
    });
    ...
    it('should throw if source email is missing and not in env', async () => {
      ...
      await expect(localModule.sendEmail({ to: 't@t.com', subject: 's', textBody: 'b' }))
    ...
    it('should resolve source email from Env vars (Priority check)', async () => {
      ...
      mockSend.mockResolvedValue({});
      await localModule.sendEmail({ to: 't@t.com', subject: 's', textBody: 'b' });

      expect(MockSendEmailCommand).toHaveBeenCalledWith(expect.objectContaining({
        Source: 'invite@test.com'
      }));
    });

    it('should throw if subject is empty', async () => {
      await expect(emailModule.sendEmail({ to: 't@t.com', subject: '  ', textBody: 'b', sourceEmail: 's@s.com' }))
        .rejects.toThrow('Email subject cannot be empty');
    });

    it('should throw if both htmlBody and textBody are missing', async () => {
      await expect(emailModule.sendEmail({ to: 't@t.com', subject: 'sub', sourceEmail: 's@s.com' }))
  ...
  describe('sendEmailTemplate', () => {
    it('should render template and call sendEmail', async () => {
      ...
      const { renderEmailTemplate } = require('../../src/utils/email-templates');
      expect(renderEmailTemplate).toHaveBeenCalledWith('organisationInvite', { some: 'data' });

      // Verify SES Command constructed with rendered data
      expect(MockSendEmailCommand).toHaveBeenCalledWith(expect.objectContaining({
        Destination: { ToAddresses: ['user@example.com'] },
        Message: expect.objectContaining({
          Subject: { Data: 'Rendered Subject for organisationInvite', Charset: 'UTF-8' },
          Body: {
            Html: { Data: '<p>HTML for organisationInvite</p>', Charset: 'UTF-8' },
