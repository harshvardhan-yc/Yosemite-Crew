...
import { DocumentService, DocumentServiceError } from '../../src/services/document.service';
...
describe('DocumentController', () => {
  ...
  describe('Internal Helpers Testing via listForPms', () => {
    ...
    it('getFirstQueryValue: should extract string from array', async () => {
      ...
      await DocumentController.listForPms(req, res);
      expect(DocumentService.listForPms).toHaveBeenCalledWith(expect.objectContaining({
        category: 'valid_string'
      }));
    });
  ...
  describe('getUploadUrl', () => {
    ...
    it('should return 200 with url and key on success', async () => {
      ...
      expect(generatePresignedUrl).toHaveBeenCalledWith('image/png', 'companion', 'c1');
  ...
  describe('createDocument', () => {
    ...
    it('should return 401 if parent profile not found', async () => {
      req.params.companionId = 'c1';
      (AuthUserMobileService.getByProviderUserId as jest.Mock).mockResolvedValue({ parentId: null });
      await DocumentController.createDocument(req, res);
      expect(res.status).toHaveBeenCalledWith(401);
      expect(res.json).toHaveBeenCalledWith({ message: 'Parent profile not found.' });
    });
    ...
    it('should return 201 on successful creation (with defaults)', async () => {
      ...
      (AuthUserMobileService.getByProviderUserId as jest.Mock).mockResolvedValue({ parentId: 'p1' });
      (DocumentService.create as jest.Mock).mockResolvedValue('created_doc');

      await DocumentController.createDocument(req, res);

      expect(DocumentService.create).toHaveBeenCalledWith(
        expect.objectContaining({ companionId: 'c1', title: 'T', category: 'C', attachments: [] }),
        { parentId: 'p1' }
      );
    ...
    it('should handle custom DocumentServiceError', async () => {
      ...
      (AuthUserMobileService.getByProviderUserId as jest.Mock).mockResolvedValue({ parentId: 'p1' });
      (DocumentService.create as jest.Mock).mockRejectedValue(new DocumentServiceError('Custom', 403));
    ...
    it('should handle generic errors', async () => {
      ...
      req.body = { title: 'T', category: 'C' };
      (AuthUserMobileService.getByProviderUserId as jest.Mock).mockResolvedValue({ parentId: 'p1' });
      (DocumentService.create as jest.Mock).mockRejectedValue(new Error('Test error'));
  ...
  describe('createDocumentPms', () => {
    ...
    it('should return 201 on success (with defaults)', async () => {
      req.params.companionId = 'c1';
      req.body = { title: 'T', category: 'C' };
      (DocumentService.create as jest.Mock).mockResolvedValue('created_pms_doc');

      await DocumentController.createDocumentPms(req, res);

      expect(DocumentService.create).toHaveBeenCalledWith(
        expect.objectContaining({ appointmentId: null }), // fallback coverage
    ...
    it('should handle custom DocumentServiceError and generic errors', async () => {
      ...
      (DocumentService.create as jest.Mock).mockRejectedValue(new DocumentServiceError('Custom', 400));
      await DocumentController.createDocumentPms(req, res);
      expect(res.status).toHaveBeenCalledWith(400);
  ...
  describe('listDocumentsForParent', () => {
    ...
    it('should handle errors', async () => {
      req.params.companionId = 'c1';
      (DocumentService.listForParent as jest.Mock).mockRejectedValue(new DocumentServiceError('Custom', 404));
      await DocumentController.listDocumentsForParent(req, res);
      expect(res.status).toHaveBeenCalledWith(404);

      (DocumentService.listForParent as jest.Mock).mockRejectedValue(new Error('Test error'));
      await DocumentController.listDocumentsForParent(req, res);
      expect(res.status).toHaveBeenCalledWith(500);
    });
  });
  ...
  describe('listForAppointment', () => {
    it('should return 200 on success', async () => {
      ...
      (DocumentService.listForAppointmentParent as jest.Mock).mockResolvedValue('app_docs');
    ...
    it('should handle errors', async () => {
      req.params.appointmentId = 'a1';
      (DocumentService.listForAppointmentParent as jest.Mock).mockRejectedValue(new DocumentServiceError('C', 400));
      await DocumentController.listForAppointment(req, res);
      expect(res.status).toHaveBeenCalledWith(400);

      (DocumentService.listForAppointmentParent as jest.Mock).mockRejectedValue(new Error('Test error'));
      await DocumentController.listForAppointment(req, res);
      expect(res.status).toHaveBeenCalledWith(500);
    });
  });
  ...
  describe('updateDocument', () => {
    ...
    it('should update successfully with parentId context', async () => {
      ...
      await DocumentController.updateDocument(req, res);
      expect(DocumentService.update).toHaveBeenCalledWith('d1', req.body, { parentId: 'p1' });
      expect(res.status).toHaveBeenCalledWith(200);
    });
    ...
    it('should update successfully with pmsUserId context (fallback param ID)', async () => {
      ...
      await DocumentController.updateDocument(req, res);
      expect(DocumentService.update).toHaveBeenCalledWith('d2', req.body, { pmsUserId: 'auth_user_123' });
      expect(res.status).toHaveBeenCalledWith(200);
    });
    ...
    it('should handle errors', async () => {
      ...
      (DocumentService.update as jest.Mock).mockRejectedValue(new DocumentServiceError('C', 404));
      await DocumentController.updateDocument(req, res);
      expect(res.status).toHaveBeenCalledWith(404);

      (DocumentService.update as jest.Mock).mockRejectedValue(new Error('Test error'));
      await DocumentController.updateDocument(req, res);
      expect(res.status).toHaveBeenCalledWith(500);
    });
  });
  ...
  describe('listForPms', () => {
    ...
    it('should handle errors', async () => {
      req.params.companionId = 'c1';
      (DocumentService.listForPms as jest.Mock).mockRejectedValue(new DocumentServiceError('C', 400));
      await DocumentController.listForPms(req, res);
      expect(res.status).toHaveBeenCalledWith(400);

      (DocumentService.listForPms as jest.Mock).mockRejectedValue(new Error('Test error'));
      await DocumentController.listForPms(req, res);
      expect(res.status).toHaveBeenCalledWith(500);
    });
  });
  ...
  describe('getForParent & getForPms', () => {
    ...
    it('getForParent: handle errors', async () => {
      req.params.id = 'd1';
      (DocumentService.getByIdForParent as jest.Mock).mockRejectedValue(new DocumentServiceError('C', 403));
      await DocumentController.getForParent(req, res);
      expect(res.status).toHaveBeenCalledWith(403);

      (DocumentService.getByIdForParent as jest.Mock).mockRejectedValue(new Error('Test error'));
      await DocumentController.getForParent(req, res);
      expect(res.status).toHaveBeenCalledWith(500);
    });
    ...
    it('getForPms: handle errors', async () => {
      req.params.documentId = 'd1';
      (DocumentService.getByIdForPms as jest.Mock).mockRejectedValue(new DocumentServiceError('C', 403));
      await DocumentController.getForPms(req, res);
      expect(res.status).toHaveBeenCalledWith(403);

      (DocumentService.getByIdForPms as jest.Mock).mockRejectedValue(new Error('Test error'));
      await DocumentController.getForPms(req, res);
      expect(res.status).toHaveBeenCalledWith(500);
    });
  });
  ...
  describe('deleteForParent', () => {
    ...
    it('should handle errors', async () => {
      ...
      (DocumentService.deleteForParent as jest.Mock).mockRejectedValue(new DocumentServiceError('C', 404));
      await DocumentController.deleteForParent(req, res);
      expect(res.status).toHaveBeenCalledWith(404);

      (DocumentService.deleteForParent as jest.Mock).mockRejectedValue(new Error('Test error'));
      await DocumentController.deleteForParent(req, res);
      expect(res.status).toHaveBeenCalledWith(500);
    });
  });
  ...
  describe('getSignedDownloadUrl', () => {
    ...
    it('should return 200 and url on success', async () => {
      req.body = { key: 'k1' };
      (generatePresignedDownloadUrl as jest.Mock).mockResolvedValue('signed_url');
      await DocumentController.getSignedDownloadUrl(req, res);
      expect(res.status).toHaveBeenCalledWith(200);
    ...
    it('should handle errors', async () => {
      req.body = { key: 'k1' };
      (generatePresignedDownloadUrl as jest.Mock).mockRejectedValue(new Error('Test error'));
      await DocumentController.getSignedDownloadUrl(req, res);
      expect(res.status).toHaveBeenCalledWith(500);
    });
  });
  ...
  describe('getDocumentDownloadUrl', () => {
    ...
    it('should return 200 and urls on success', async () => {
      req.params.documentId = 'd1';
      (DocumentService.getAllAttachmentUrls as jest.Mock).mockResolvedValue(['url1']);
      await DocumentController.getDocumentDownloadUrl(req, res);
      expect(res.status).toHaveBeenCalledWith(200);
    ...
    it('should handle errors', async () => {
      ...
      (DocumentService.getAllAttachmentUrls as jest.Mock).mockRejectedValue(new DocumentServiceError('C', 403));
      await DocumentController.getDocumentDownloadUrl(req, res);
      expect(res.status).toHaveBeenCalledWith(403);

      (DocumentService.getAllAttachmentUrls as jest.Mock).mockRejectedValue(new Error('Test error'));
      await DocumentController.getDocumentDownloadUrl(req, res);
      expect(res.status).toHaveBeenCalledWith(500);
    });
  });
  ...
  describe('searchDocument', () => {
    ...
    it('should return 200 and results on success', async () => {
      ...
      (DocumentService.searchByTitleForParent as jest.Mock).mockResolvedValue(['doc1']);
      await DocumentController.searchDocument(req, res);
      expect(res.status).toHaveBeenCalledWith(200);
    ...
    it('should handle errors', async () => {
      ...
      (DocumentService.searchByTitleForParent as jest.Mock).mockRejectedValue(new DocumentServiceError('C', 400));
      await DocumentController.searchDocument(req, res);
      expect(res.status).toHaveBeenCalledWith(400);

      (DocumentService.searchByTitleForParent as jest.Mock).mockRejectedValue(new Error('Test error'));
      await DocumentController.searchDocument(req, res);
      expect(res.status).toHaveBeenCalledWith(500);
    });
  });
