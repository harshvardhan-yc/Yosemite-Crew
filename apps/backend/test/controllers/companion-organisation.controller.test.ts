...
describe('CompanionOrganisationController', () => {
  ...
  describe('linkByParent', () => {
    ...
    it('should successfully link by parent', async () => {
      ...
      await CompanionOrganisationController.linkByParent(req, res);
      expect(CompanionOrganisationService.linkByParent).toHaveBeenCalledWith({ parentId: 'pid', ...req.body });
  ...
  describe('linkByPmsUser', () => {
    ...
    it('should return 404 if org not found or invalid type', async () => {
      ...
      // Invalid type
      (OrganizationModel.findById as jest.Mock).mockResolvedValue({ type: 'INVALID' });
      await CompanionOrganisationController.linkByPmsUser(req, res);
      expect(res.status).toHaveBeenCalledWith(404);
    });
    ...
    it('should successfully link by PMS user', async () => {
      req.params = { companionId: 'c1', organisationId: 'o1' };
      (OrganizationModel.findById as jest.Mock).mockResolvedValue({ type: 'BREEDER' });
      (CompanionOrganisationService.linkByPmsUser as jest.Mock).mockResolvedValue('link_data');
    ...
    it('should handle errors', async () => {
      ...
      // Generic
      (CompanionOrganisationService.linkByPmsUser as jest.Mock).mockRejectedValue(new Error('Test error'));
      await CompanionOrganisationController.linkByPmsUser(req, res);
      expect(res.status).toHaveBeenCalledWith(500);

      // Custom
      (CompanionOrganisationService.linkByPmsUser as jest.Mock).mockRejectedValue(new CompanionOrganisationServiceError('Custom', 400));
      await CompanionOrganisationController.linkByPmsUser(req, res);
      expect(res.status).toHaveBeenCalledWith(400);
    });
  });
  ...
  describe('approvePendingLink', () => {
    ...
    it('should approve link on success', async () => {
      ...
      (CompanionOrganisationService.parentApproveLink as jest.Mock).mockResolvedValue('approved_data');
    ...
    it('should handle errors', async () => {
      ...
      (CompanionOrganisationService.parentApproveLink as jest.Mock).mockRejectedValue(new Error('err'));
      await CompanionOrganisationController.approvePendingLink(req, res);
      expect(res.status).toHaveBeenCalledWith(500);

      (CompanionOrganisationService.parentApproveLink as jest.Mock).mockRejectedValue(new CompanionOrganisationServiceError('err', 403));
      await CompanionOrganisationController.approvePendingLink(req, res);
  ...
  describe('sendInvite', () => {
    ...
    it('should succeed when email is provided', async () => {
      ...
      await CompanionOrganisationController.sendInvite(req, res);
      expect(CompanionOrganisationService.sendInvite).toHaveBeenCalledWith(expect.objectContaining({ email: 'test@test.com' }));
      expect(res.status).toHaveBeenCalledWith(201);
    });

    it('should succeed when name is provided', async () => {
      (ParentService.findByLinkedUserId as jest.Mock).mockResolvedValue({ _id: 'pid' });
      req.body = { companionId: 'c1', organisationType: 'HOSPITAL', name: ' OrgName ' };
      await CompanionOrganisationController.sendInvite(req, res);
      expect(CompanionOrganisationService.sendInvite).toHaveBeenCalledWith(expect.objectContaining({ name: 'OrgName' }));
      expect(res.status).toHaveBeenCalledWith(201);
    });

    it('should succeed when placesId is provided', async () => {
      (ParentService.findByLinkedUserId as jest.Mock).mockResolvedValue({ _id: 'pid' });
      req.body = { companionId: 'c1', organisationType: 'HOSPITAL', placesId: ' place123 ' };
      await CompanionOrganisationController.sendInvite(req, res);
      expect(CompanionOrganisationService.sendInvite).toHaveBeenCalledWith(expect.objectContaining({ placesId: 'place123' }));
      expect(res.status).toHaveBeenCalledWith(201);
    });

    it('should handle errors', async () => {
      (ParentService.findByLinkedUserId as jest.Mock).mockResolvedValue({ _id: 'pid' });
      req.body = { companionId: 'c1', organisationType: 'HOSPITAL', email: 'e' };

      (CompanionOrganisationService.sendInvite as jest.Mock).mockRejectedValue(new Error('err'));
      await CompanionOrganisationController.sendInvite(req, res);
      expect(res.status).toHaveBeenCalledWith(500);

      (CompanionOrganisationService.sendInvite as jest.Mock).mockRejectedValue(new CompanionOrganisationServiceError('err', 400));
      await CompanionOrganisationController.sendInvite(req, res);
      expect(res.status).toHaveBeenCalledWith(400);
    });
  });
  ...
  describe('denyPendingLink', () => {
    ...
    it('should deny link on success', async () => {
      (ParentService.findByLinkedUserId as jest.Mock).mockResolvedValue({ _id: 'pid' });
      req.params.linkId = 'link_1';
      (CompanionOrganisationService.parentRejectLink as jest.Mock).mockResolvedValue('denied_data');
    ...
    it('should handle errors', async () => {
      ...
      (CompanionOrganisationService.parentRejectLink as jest.Mock).mockRejectedValue(new Error('err'));
      await CompanionOrganisationController.denyPendingLink(req, res);
      expect(res.status).toHaveBeenCalledWith(500);

      (CompanionOrganisationService.parentRejectLink as jest.Mock).mockRejectedValue(new CompanionOrganisationServiceError('err', 404));
      await CompanionOrganisationController.denyPendingLink(req, res);
      expect(res.status).toHaveBeenCalledWith(404);
    });
  });
  ...
  describe('acceptInvite & rejectInvite (testing parseInviteResolutionPayload)', () => {
    ...
    it('should accept invite on success', async () => {
      ...
      (CompanionOrganisationService.acceptInvite as jest.Mock).mockResolvedValue('acc_data');
      await CompanionOrganisationController.acceptInvite(req, res);
      expect(res.status).toHaveBeenCalledWith(200);
    });
    ...
    it('should handle errors in acceptInvite', async () => {
      req.body = { token: 't1', organisationId: 'o1' };
      (CompanionOrganisationService.acceptInvite as jest.Mock).mockRejectedValue(new Error('Test error'));
      await CompanionOrganisationController.acceptInvite(req, res);
      expect(res.status).toHaveBeenCalledWith(500);

      (CompanionOrganisationService.acceptInvite as jest.Mock).mockRejectedValue(new CompanionOrganisationServiceError('e', 400));
      await CompanionOrganisationController.acceptInvite(req, res);
      expect(res.status).toHaveBeenCalledWith(400);
    });
    ...
    it('should handle errors in rejectInvite', async () => {
      req.body = { token: 't1', organisationId: 'o1' };
      (CompanionOrganisationService.rejectInvite as jest.Mock).mockRejectedValue(new Error('Test error'));
      await CompanionOrganisationController.rejectInvite(req, res);
      expect(res.status).toHaveBeenCalledWith(500);

      (CompanionOrganisationService.rejectInvite as jest.Mock).mockRejectedValue(new CompanionOrganisationServiceError('e', 400));
      await CompanionOrganisationController.rejectInvite(req, res);
      expect(res.status).toHaveBeenCalledWith(400);
    });
  });
  ...
  describe('revokeLink', () => {
    it('should revoke link on success', async () => {
      ...
      (CompanionOrganisationService.revokeLink as jest.Mock).mockResolvedValue('revoked');
    ...
    it('should handle errors', async () => {
      (CompanionOrganisationService.revokeLink as jest.Mock).mockRejectedValue(new Error('Test error'));
      await CompanionOrganisationController.revokeLink(req, res);
      expect(res.status).toHaveBeenCalledWith(500);

      (CompanionOrganisationService.revokeLink as jest.Mock).mockRejectedValue(new CompanionOrganisationServiceError('Test error', 403));
      await CompanionOrganisationController.revokeLink(req, res);
      expect(res.status).toHaveBeenCalledWith(403);
    });
  });
  ...
  describe('getLinksForCompanion', () => {
    it('should fetch links successfully', async () => {
      ...
      (CompanionOrganisationService.getLinksForCompanion as jest.Mock).mockResolvedValue([]);
      await CompanionOrganisationController.getLinksForCompanion(req, res);
      expect(res.status).toHaveBeenCalledWith(200);
    });

    it('should handle errors', async () => {
      (CompanionOrganisationService.getLinksForCompanion as jest.Mock).mockRejectedValue(new Error('Test error'));
      await CompanionOrganisationController.getLinksForCompanion(req, res);
      expect(res.status).toHaveBeenCalledWith(500);
    });
  });
  ...
  describe('getLinksForOrganisation', () => {
    it('should fetch links successfully', async () => {
      ...
      (CompanionOrganisationService.getLinksForOrganisation as jest.Mock).mockResolvedValue([]);
      await CompanionOrganisationController.getLinksForOrganisation(req, res);
      expect(res.status).toHaveBeenCalledWith(200);
    });

    it('should handle errors', async () => {
      (CompanionOrganisationService.getLinksForOrganisation as jest.Mock).mockRejectedValue(new Error('Test error'));
      await CompanionOrganisationController.getLinksForOrganisation(req, res);
      expect(res.status).toHaveBeenCalledWith(500);
    });
  });
  ...
  describe('getLinksForCompanionByOrganisationType', () => {
    it('should return 400 if companionId missing', async () => {
      ...
      await CompanionOrganisationController.getLinksForCompanionByOrganisationType(req, res);
      expect(res.status).toHaveBeenCalledWith(400);
    });
    ...
    it('should fetch links successfully', async () => {
      req.params.companionId = 'c1';
      req.query.type = 'HOSPITAL';
      (CompanionOrganisationService.getLinksForCompanionByOrganisationTye as jest.Mock).mockResolvedValue([]);

      await CompanionOrganisationController.getLinksForCompanionByOrganisationType(req, res);
      expect(res.status).toHaveBeenCalledWith(200);
      expect(CompanionOrganisationService.getLinksForCompanionByOrganisationTye).toHaveBeenCalledWith('c1', 'HOSPITAL');
    });
    ...
    it('should handle errors', async () => {
      ...
      (CompanionOrganisationService.getLinksForCompanionByOrganisationTye as jest.Mock).mockRejectedValue(new Error('Test error'));
      await CompanionOrganisationController.getLinksForCompanionByOrganisationType(req, res);
      expect(res.status).toHaveBeenCalledWith(500);

      (CompanionOrganisationService.getLinksForCompanionByOrganisationTye as jest.Mock).mockRejectedValue(new CompanionOrganisationServiceError('err', 404));
      await CompanionOrganisationController.getLinksForCompanionByOrganisationType(req, res);
      expect(res.status).toHaveBeenCalledWith(404);
    });
  });
