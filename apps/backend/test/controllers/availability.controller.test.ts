...
describe('AvailabilityController', () => {
  ...
  describe('Internal Helpers (Implicitly Tested)', () => {
    it('resolveUserIdFromRequest: should use x-user-id header if present', async () => {
      ...
      expect(AvailabilityService.getBaseAvailability).toHaveBeenCalledWith('org1', 'header_user_id');
    });

    it('handleControllerError: should fallback to "Internal server error" if err is not an Error instance', async () => {
      req.params.orgId = 'org1';
      (AvailabilityService.getBaseAvailability as jest.Mock).mockRejectedValue('String Error');

      // Removed "as Request" and "as Response"
      await AvailabilityController.getBaseAvailability(req, res);

      expect(logger.error).toHaveBeenCalledWith('getBaseAvailability error', 'String Error');
  ...
  describe('Base Availability', () => {
    describe('setAllBaseAvailability', () => {
      ...
      it('should return 201 and data on success', async () => {
        ...
        (AvailabilityService.setAllBaseAvailability as jest.Mock).mockResolvedValue('mockData');

        await AvailabilityController.setAllBaseAvailability(req, res);

        expect(AvailabilityService.setAllBaseAvailability).toHaveBeenCalledWith('org1', 'auth_user_123', req.body.availabilities);
        expect(res.status).toHaveBeenCalledWith(201);
        expect(res.json).toHaveBeenCalledWith({ message: 'Base availability saved', data: 'mockData' });
      });
      ...
      it('should handle errors', async () => {
        ...
        (AvailabilityService.setAllBaseAvailability as jest.Mock).mockRejectedValue(err);

        await AvailabilityController.setAllBaseAvailability(req, res);

        expect(logger.error).toHaveBeenCalledWith('setAllBaseAvailability error', err);
        expect(res.status).toHaveBeenCalledWith(500);
    ...
    describe('getBaseAvailability', () => {
      it('should return 400 if orgId is missing', async () => {
        await AvailabilityController.getBaseAvailability(req, res);
        expect(res.status).toHaveBeenCalledWith(400);
        expect(res.json).toHaveBeenCalledWith({ message: 'Missing orgId or userId' });
      });

      it('should return 200 and data on success', async () => {
        req.params.orgId = 'org1';
        (AvailabilityService.getBaseAvailability as jest.Mock).mockResolvedValue('baseData');

        await AvailabilityController.getBaseAvailability(req, res);

        expect(AvailabilityService.getBaseAvailability).toHaveBeenCalledWith('org1', 'auth_user_123');
    ...
    describe('deleteBaseAvailability', () => {
      ...
      it('should return 200 on success', async () => {
        req.params.orgId = 'org1';
        await AvailabilityController.deleteBaseAvailability(req, res);
        expect(AvailabilityService.deleteBaseAvailability).toHaveBeenCalledWith('org1', 'auth_user_123');
        expect(res.status).toHaveBeenCalledWith(200);
        expect(res.json).toHaveBeenCalledWith({ message: 'Base availability deleted' });
      });

      it('should handle errors', async () => {
        req.params.orgId = 'org1';
        (AvailabilityService.deleteBaseAvailability as jest.Mock).mockRejectedValue(new Error('Del Error'));
        await AvailabilityController.deleteBaseAvailability(req, res);
        expect(res.status).toHaveBeenCalledWith(500);
      });
    });
  ...
  describe('Weekly Overrides', () => {
    describe('addWeeklyAvailabilityOverride', () => {
      ...
      it('should return 201 on success (tests safeDate with Date object)', async () => {
        ...
        expect(AvailabilityService.addWeeklyAvailabilityOverride).toHaveBeenCalledWith(
          'org1', 'auth_user_123', dateObj, req.body.overrides
        );
        expect(res.status).toHaveBeenCalledWith(201);
      });
      ...
      it('should handle errors', async () => {
        ...
        (AvailabilityService.addWeeklyAvailabilityOverride as jest.Mock).mockRejectedValue(new Error('err'));
        await AvailabilityController.addWeeklyAvailabilityOverride(req, res);
        expect(res.status).toHaveBeenCalledWith(500);
      });
    });
    ...
    describe('getWeeklyAvailabilityOverride', () => {
      ...
      it('should return 404 if no override found', async () => {
        ...
        (AvailabilityService.getWeeklyAvailabilityOverride as jest.Mock).mockResolvedValue(null);
      ...
      it('should return 200 and data if found', async () => {
        req.params.orgId = 'org1';
        req.query.weekStartDate = '2026-03-07';
        (AvailabilityService.getWeeklyAvailabilityOverride as jest.Mock).mockResolvedValue('overrideData');
      ...
      it('should handle errors', async () => {
        req.params.orgId = 'org1';
        req.query.weekStartDate = '2026-03-07';
        (AvailabilityService.getWeeklyAvailabilityOverride as jest.Mock).mockRejectedValue(new Error('err'));
        await AvailabilityController.getWeeklyAvailabilityOverride(req, res);
        expect(res.status).toHaveBeenCalledWith(500);
      });
    });
    ...
    describe('deleteWeeklyAvailabilityOverride', () => {
      ...
      it('should handle errors', async () => {
        req.params.orgId = 'org1';
        req.query.weekStartDate = '2026-03-07';
        (AvailabilityService.deleteWeeklyAvailabilityOverride as jest.Mock).mockRejectedValue(new Error('err'));
        await AvailabilityController.deleteWeeklyAvailabilityOverride(req, res);
        expect(res.status).toHaveBeenCalledWith(500);
      });
    });
  ...
  describe('Occupancy Blocks', () => {
    describe('addOccupancy', () => {
      ...
      it('should handle errors', async () => {
        req.params.orgId = 'org1';
        req.body = { startTime: '2026-03-07T10:00:00Z', endTime: '2026-03-07T11:00:00Z', sourceType: 'BLOCK' };
        (AvailabilityService.addOccupancy as jest.Mock).mockRejectedValue(new Error('err'));
        await AvailabilityController.addOccupancy(req, res);
        expect(res.status).toHaveBeenCalledWith(500);
      });
    });
    ...
    describe('addAllOccupancies', () => {
      ...
      it('should return 400 if inner loop encounters an invalid date', async () => {
        req.body = {
          ...
          userId: 'u1',
          occupancies: [
            { startTime: 'invalid', endTime: '2026-03-07T11:00:00Z', sourceType: 'BLOCK' }
          ]
        };
        await AvailabilityController.addAllOccupancies(req, res);
        expect(res.status).toHaveBeenCalledWith(400);
        expect(res.json).toHaveBeenCalledWith({ message: 'Invalid occupancy payload' });
      });
      ...
      it('should return 201 on success mapping multiple occupancies', async () => {
        req.body = {
          ...
          userId: 'u1',
          occupancies: [
            { startTime: '2026-03-07T10:00:00Z', endTime: '2026-03-07T11:00:00Z', sourceType: 'BLOCK', referenceId: 'ref1' },
            { startTime: '2026-03-08T10:00:00Z', endTime: '2026-03-08T11:00:00Z', sourceType: 'APPOINTMENT' }
          ]
        };
      ...
      it('should handle errors', async () => {
        req.body = {
          organisationId: 'org1', userId: 'u1',
          occupancies: [{ startTime: '2026-03-07T10:00:00Z', endTime: '2026-03-07T11:00:00Z', sourceType: 'BLOCK' }]
        };
        (AvailabilityService.addAllOccupancies as jest.Mock).mockRejectedValue(new Error('err'));
        await AvailabilityController.addAllOccupancies(req, res);
        expect(res.status).toHaveBeenCalledWith(500);
      });
    });
    ...
    describe('getOccupancy', () => {
      ...
      it('should return 200 and data on success', async () => {
        ...
        (AvailabilityService.getOccupancy as jest.Mock).mockResolvedValue('occData');
      ...
      it('should handle errors', async () => {
        ...
        req.query.startDate = '2026-03-01';
        req.query.endDate = '2026-03-31';
        (AvailabilityService.getOccupancy as jest.Mock).mockRejectedValue(new Error('err'));
        await AvailabilityController.getOccupancy(req, res);
        expect(res.status).toHaveBeenCalledWith(500);
      });
    });
  ...
  describe('Final Availability & Status', () => {
    describe('getFinalAvailability', () => {
      ...
      it('should return 200 and data on success', async () => {
        ...
        (AvailabilityService.getFinalAvailabilityForDate as jest.Mock).mockResolvedValue('finalData');
      ...
      it('should handle errors', async () => {
        req.params.orgId = 'org1';
        req.query.referenceDate = '2026-03-07';
        (AvailabilityService.getFinalAvailabilityForDate as jest.Mock).mockRejectedValue(new Error('err'));
        await AvailabilityController.getFinalAvailability(req, res);
        expect(res.status).toHaveBeenCalledWith(500);
      });
    });
    ...
    describe('getCurrentStatus', () => {
      ...
      it('should return 200 and status on success', async () => {
        req.params.orgId = 'org1';
        (AvailabilityService.getCurrentStatus as jest.Mock).mockResolvedValue('statusData');
      ...
      it('should handle errors', async () => {
        req.params.orgId = 'org1';
        (AvailabilityService.getCurrentStatus as jest.Mock).mockRejectedValue(new Error('err'));
        await AvailabilityController.getCurrentStatus(req, res);
        expect(res.status).toHaveBeenCalledWith(500);
      });
    });
