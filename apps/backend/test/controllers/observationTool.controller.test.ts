...
const { ObservationToolDefinitionServiceError: RealDefError } = jest.requireActual('../../src/services/observationToolDefinition.service') as any;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { ObservationToolSubmissionServiceError: RealSubError } = jest.requireActual('../../src/services/observationToolSubmission.service') as any;
...
describe('ObservationTool Controllers', () => {
  ...
  // ----------------------------------------------------------------------
  const mockDefError = (method: keyof typeof ObservationToolDefinitionService, status = 400, msg = 'Def Error') => {
    const error = new RealDefError(msg, status);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (mockedDefService[method] as any).mockRejectedValue(error);
  };

  const mockSubError = (method: keyof typeof ObservationToolSubmissionService, status = 400, msg = 'Sub Error') => {
  ...
  describe('ObservationToolDefinitionController', () => {
    ...
    describe('create', () => {
      it('should success (201)', async () => {
        ...
        await ObservationToolDefinitionController.create(req as any, res as Response);
    ...
    describe('update', () => {
      it('should success (200)', async () => {
        ...
        await ObservationToolDefinitionController.update(req as any, res as Response);
    ...
    describe('archive', () => {
      it('should success (204)', async () => {
        ...
        await ObservationToolDefinitionController.archive(req as any, res as Response);
    ...
    describe('list', () => {
      it('should list with filters', async () => {
        ...
        await ObservationToolDefinitionController.list(req as any, res as Response);
        expect(mockedDefService.list).toHaveBeenCalledWith({ category: 'cat1', onlyActive: true });
      ...
      it('should list with numeric active flag', async () => {
        ...
        await ObservationToolDefinitionController.list(req as any, res as Response);
        expect(mockedDefService.list).toHaveBeenCalledWith({ category: undefined, onlyActive: true });
      });
    ...
    describe('getById', () => {
      it('should success', async () => {
        ...
        await ObservationToolDefinitionController.getById(req as any, res as Response);
  ...
  describe('ObservationToolSubmissionController', () => {
    ...
    describe('createFromMobile', () => {
      it('should 401 if unauthenticated', async () => {
        await ObservationToolSubmissionController.createFromMobile(req as any, res as Response);
      ...
      it('should success (201)', async () => {
        ...
        (mockedAuthService.getByProviderUserId as any).mockResolvedValue({ parentId: 'p1' });
        req.params = { toolId: 't1' };
        req.body = { companionId: 'c1', answers: { q1: 'a1' }, summary: 'sum', taskId: 'tsk1' };

        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (mockedSubService.createSubmission as any).mockResolvedValue({ id: 's1' });
    ...
    describe('listForPms', () => {
      it('should list with full filters', async () => {
        req.query = { companionId: 'c1', toolId: 't1', fromDate: '2023-01-01', toDate: '2023-01-31' };
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (mockedSubService.listSubmissions as any).mockResolvedValue([]);

        await ObservationToolSubmissionController.listForPms(req as any, res as Response);
      ...
      it('should list with minimal filters', async () => {
        ...
        await ObservationToolSubmissionController.listForPms(req as any, res as Response);
        expect(mockedSubService.listSubmissions).toHaveBeenCalledWith({
            companionId: undefined, toolId: undefined, fromDate: undefined, toDate: undefined
        });
      });
    ...
    describe('getById', () => {
      it('should 404 if not found', async () => {
        ...
        await ObservationToolSubmissionController.getById(req as any, res as Response);
    ...
    describe('linkAppointment', () => {
      it('should link and return updated', async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (mockedSubService.linkToAppointment as any).mockResolvedValue({ taskId: 'tsk1' });
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (mockedTaskService.linkToAppointment as any).mockResolvedValue(undefined);

        await ObservationToolSubmissionController.linkAppointment(req as any, res as Response);

        expect(mockedSubService.linkToAppointment).toHaveBeenCalledWith({
            submissionId: 's1', appointmentId: 'apt1', enforceSingleSubmissionPerAppointment: true
        });
        expect(mockedTaskService.linkToAppointment).toHaveBeenCalledWith({ taskId: 'tsk1', appointmentId: 'apt1' });
    ...
    describe('listForAppointment', () => {
      it('should success', async () => {
        ...
        await ObservationToolSubmissionController.listForAppointment(req as any, res as Response);
        expect(mockedSubService.listForAppointment).toHaveBeenCalledWith('apt1');
      });
    ...
    describe('getByTaskId', () => {
      it('should 404 if not found', async () => {
        ...
        await ObservationToolSubmissionController.getByTaskId(req as any, res as Response);
        expect(statusMock).toHaveBeenCalledWith(404);
      });
    ...
    describe('getPreviewByTaskId', () => {
      it('should success', async () => {
        ...
        await ObservationToolSubmissionController.getPreviewByTaskId(req as any, res as Response);
        expect(jsonMock).toHaveBeenCalled();
      });
    ...
    describe('listTaskPreviewsForAppointment', () => {
      it('should success', async () => {
        req.params = { appointmentId: 'apt1' };
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (mockedSubService.listTaskPreviewsForAppointment as any).mockResolvedValue([]);
        await ObservationToolSubmissionController.listTaskPreviewsForAppointment(req as any, res as Response);
        expect(jsonMock).toHaveBeenCalledWith([]);
      });
