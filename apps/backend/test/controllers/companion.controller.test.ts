...
describe("CompanionController", () => {
  ...
  describe("createCompanionMobile", () => {
    ...
    it("creates companion when authenticated", async () => {
      ...
      const res = createResponse();
      mockedCompanionService.create.mockResolvedValueOnce({
        response: { id: "cmp-1" },
      });
    ...
    it("maps CompanionServiceError errors", async () => {
      const req = { body: patientPayload, userId: "user-1" } as any;
      const res = createResponse();
      mockedCompanionService.create.mockRejectedValueOnce(
        new CompanionServiceError("bad", 422),
      );
  ...
  describe("getCompanionById", () => {
    ...
    it("returns companion response", async () => {
      const req = { params: { id: "cmp-1" } } as any;
      const res = createResponse();
      mockedCompanionService.getById.mockResolvedValueOnce({
        response: { id: "cmp-1" },
      });
  ...
  describe("updateCompanion", () => {
    ...
    it("updates companion", async () => {
      ...
      const res = createResponse();
      mockedCompanionService.update.mockResolvedValueOnce({
        response: { id: "cmp-1", name: "Updated" },
      });
    ...
    it("maps CompanionServiceError", async () => {
      const req = { params: { id: "cmp-1" }, body: patientPayload } as any;
      const res = createResponse();
      mockedCompanionService.update.mockRejectedValueOnce(
        new CompanionServiceError("nope", 422),
      );
  ...
  describe("deleteCompanion", () => {
    ...
    it("maps CompanionServiceError", async () => {
      ...
      const res = createResponse();
      mockedCompanionService.delete.mockRejectedValueOnce(
        new CompanionServiceError("boom", 403),
      );
