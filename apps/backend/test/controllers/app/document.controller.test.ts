...
jest.mock('../../../src/services/document.service', () => {
  ...
  const actual = jest.requireActual('../../../src/services/document.service') as unknown as any;
  return {
    ...actual,
    DocumentService: {
      create: jest.fn(),
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const { DocumentServiceError } = jest.requireActual('../../../src/services/document.service') as unknown as any;
...
const mockGenericError = (
  ...
  describe('createDocument (Mobile)', () => {
    ...
    it('should 401 if parent profile not found', async () => {
      ...
      (req as any).userId = 'u1';
      mockedAuthMobileService.getByProviderUserId.mockResolvedValue({ parentId: null } as any);
    ...
    it('should success (201)', async () => {
      ...
      mockedAuthMobileService.getByProviderUserId.mockResolvedValue({ parentId: 'p1' } as any);
  ...
  describe('listDocumentsForParent', () => {
    it('should 400 if companionId missing', async () => {
      req.params = {};
      await DocumentController.listDocumentsForParent(req as any, res as Response);
      expect(statusMock).toHaveBeenCalledWith(400);
    });
  ...
  describe('listForAppointment', () => {
    it('should success (200)', async () => {
      ...
      mockedDocumentService.listForAppointmentParent.mockResolvedValue([] as any);
  ...
  describe('getForParent', () => {
    ...
    it('should success (200)', async () => {
      req.params = { id: 'd1' };
      mockedDocumentService.getByIdForParent.mockResolvedValue({ id: 'd1' } as any);
      await DocumentController.getForParent(req as any, res as Response);
  ...
  describe('getForPms', () => {
    ...
    it('should success (200)', async () => {
      req.params = { documentId: 'd1' };
      mockedDocumentService.getByIdForPms.mockResolvedValue({ id: 'd1' } as any);
      await DocumentController.getForPms(req as any, res as Response);
      expect(res.json).toHaveBeenCalledWith({ id: 'd1' });
    });
  ...
  describe('getSignedDownloadUrl', () => {
    it('should 400 if key missing', async () => {
      req.body = {};
      await DocumentController.getSignedDownloadUrl(req as any, res as Response);
      expect(statusMock).toHaveBeenCalledWith(400);
    });

    it('should success (200)', async () => {
      req.body = { key: 'k1' };
      mockedUpload.generatePresignedDownloadUrl.mockResolvedValue('http://d/k1');
      await DocumentController.getSignedDownloadUrl(req as any, res as Response);
      expect(statusMock).toHaveBeenCalledWith(200);
    ...
    it('should handle generic error', async () => {
      req.body = { key: 'k1' };
      mockedUpload.generatePresignedDownloadUrl.mockRejectedValue(new Error('fail'));
      await DocumentController.getSignedDownloadUrl(req as any, res as Response);
      expect(statusMock).toHaveBeenCalledWith(500);
    });
  });
  ...
  describe('getDocumentDownloadUrl', () => {
    it('should 400 if doc id missing', async () => {
      req.params = {};
      await DocumentController.getDocumentDownloadUrl(req as any, res as Response);
      expect(statusMock).toHaveBeenCalledWith(400);
    });
