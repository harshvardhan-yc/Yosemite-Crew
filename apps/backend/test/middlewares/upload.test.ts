...
describe("Upload Middleware", () => {
  ...
  describe("Utilities & Configuration", () => {
    ...
    it("should throw if CloudFront URL is missing", () => {
      ...
      expect(() => UploadMiddleware.getURLForKey("key")).toThrow(
    ...
    it("generatePresignedDownloadUrl should return CF URL", async () => {
      const url =
        await UploadMiddleware.generatePresignedDownloadUrl("key.jpg");
    ...
    describe("buildS3Key", () => {
      it("should build keys for all types correctly", () => {
        expect(
          UploadMiddleware.buildS3Key("temp", undefined, "image/png"),
        ).toBe("temp/uploads/mock-uuid.png");
        expect(UploadMiddleware.buildS3Key("user", "123", "image/jpeg")).toBe(
          "users/123/mock-uuid.jpg",
        );
        expect(UploadMiddleware.buildS3Key("org", "456")).toBe(
          "orgs/456/mock-uuid",
        );
        expect(UploadMiddleware.buildS3Key("parent", "p1")).toBe(
          "parent/p1/mock-uuid",
        );
        expect(UploadMiddleware.buildS3Key("companion", "c1")).toBe(
          "companion/c1/mock-uuid",
        );
        expect(UploadMiddleware.buildS3Key("custom", "folder")).toBe(
          "folder/mock-uuid",
        );
        expect(UploadMiddleware.buildS3Key("user-org", "uo1")).toBe(
          "user-org/uo1/mock-uuid",
        );
      });

      it("should throw error for invalid type", () => {
        // @ts-ignore - Testing runtime validation
        expect(() => UploadMiddleware.buildS3Key("invalid", "123")).toThrow(
          "Invalid upload type",
        );
      });
    });
  ...
  describe("Upload Handlers", () => {
    describe("uploadToS3 (Internal)", () => {
      it("should upload successfully", async () => {
        const mockPromise = (jest.fn() as any).mockResolvedValue({
          Location: "s3-loc",
          Key: "key",
        });
        ...
        const result = await UploadMiddleware.handleFileUpload(
          file,
          "test-folder",
        );
      ...
      it("should handle S3 upload failure", async () => {
        const mockPromise = (jest.fn() as any).mockRejectedValue(
        ...
        await expect(
          UploadMiddleware.handleFileUpload(file, "test-folder"),
    ...
    describe("handleFileUpload", () => {
      ...
      it("should throw if unsupported mime type", async () => {
        ...
        await expect(
          UploadMiddleware.handleFileUpload(file, "folder"),
    ...
    describe("handleMultipleFileUpload", () => {
      it("should upload multiple files", async () => {
        ...
        const results = await UploadMiddleware.handleMultipleFileUpload(
          files,
          "multi",
        );
    ...
    describe("uploadBufferAsFile", () => {
      ...
      it("should throw on invalid mime", async () => {
        await expect(
          UploadMiddleware.uploadBufferAsFile(Buffer.from(""), {
            folderName: "f",
            mimeType: "bad/mime",
          }),
        ).rejects.toThrow("Unsupported file type");
  ...
  describe("S3 Operations", () => {
    describe("generatePresignedUrl", () => {
      it("should return url and key", async () => {
        ...
        (mockS3Instance.getSignedUrlPromise as any).mockResolvedValue(
          "http://signed.url",
        );

        const result = await UploadMiddleware.generatePresignedUrl(
          "image/jpeg",
          "user",
          "123",
        );
      ...
      it("should handle errors", async () => {
        // Cast to any to fix type error
        (mockS3Instance.getSignedUrlPromise as any).mockRejectedValue(
          new Error("Signing failed"),
        );
        await expect(
          UploadMiddleware.generatePresignedUrl("image/jpeg", "temp"),
    ...
    describe("moveFile", () => {
      ...
      it("should handle errors during move", async () => {
        ...
        await expect(UploadMiddleware.moveFile("old", "new")).rejects.toThrow(
          "Failed to move file: Copy failed",
        );
      });
    });
    ...
    describe("deleteFromS3", () => {
      ...
      it("should throw error on failure", async () => {
        ...
        await expect(UploadMiddleware.deleteFromS3("key")).rejects.toThrow(
          "Del failed",
        );
      });
    });
  ...
  describe("setupLifecyclePolicy", () => {
    it("should do nothing if rule already exists", async () => {
      ...
      expect(
        mockS3Instance.putBucketLifecycleConfiguration,
    ...
    it("should throw on generic error", async () => {
      ...
      await expect(UploadMiddleware.setupLifecyclePolicy()).rejects.toThrow(
        "Failed to set lifecycle policy: Access Denied",
      );
    });
    ...
    it("should handle error object with null code in getErrorCode helper", async () => {
      ...
      await expect(UploadMiddleware.setupLifecyclePolicy()).rejects.toThrow(
        "Failed to set lifecycle policy: Unknown error",
      );
    });
  });
});
