...
describe("UserOrganizationService", () => {
  ...
  beforeEach(() => {
    ...
    (UserOrganizationModel.findOne as jest.Mock).mockReturnValue(mockChain(null));
    ...
    (UserOrganizationModel.create as jest.Mock).mockResolvedValue(mockDoc({ ...validPayload, _id: mockMappingId }));

    (OrganizationModel.findOne as jest.Mock).mockReturnValue(mockChain({ _id: mockOrgId, name: "Test Org" }));
    (OrgUsageCounters.findOneAndUpdate as jest.Mock).mockReturnValue(mockChain({ _id: "counter-1" }));
  ...
  describe("Validation & Internals", () => {
    ...
    it("should throw error for invalid Role Code", async () => {
      ...
      await expect(UserOrganizationService.create(invalidPayload)).rejects.toThrow(
        'Invalid roleCode "INVALID_ROLE"'
      );
    });

    it("should handle Date objects in pruning", async () => {
      const payloadWithDate = { ...validPayload, period: { start: new Date() } };
  ...
  describe("upsert", () => {
    ...
    it("should rollback slot reservation if creation fails", async () => {
      (UserOrganizationModel.findOne as jest.Mock).mockReturnValue(mockChain(null));
      (UserOrganizationModel.create as jest.Mock).mockRejectedValue(new Error("DB Error"));

      await expect(UserOrganizationService.upsert(validPayload)).rejects.toThrow("DB Error");
    ...
    it("should update existing mapping: Active -> Inactive (Release Slot)", async () => {
      ...
      (UserOrganizationModel.findOne as jest.Mock).mockReturnValue(mockChain(mockDoc(existing)));
    ...
    it("should sync seats if business plan and slot count changed", async () => {
      (OrgBilling.findOne as jest.Mock).mockReturnValue(mockChain({ plan: "business" }));
      (UserOrganizationModel.findOne as jest.Mock).mockReturnValue(mockChain(null));
    ...
    it("should throw error if findOneAndUpdate returns null during upsert", async () => {
      ...
      (UserOrganizationModel.findOneAndUpdate as jest.Mock).mockResolvedValue(null);

      await expect(UserOrganizationService.upsert(validPayload)).rejects.toThrow(
  ...
  describe("create", () => {
    it("should fail creation and rollback if error occurs", async () => {
      (UserOrganizationModel.create as jest.Mock).mockRejectedValue(new Error("Fail"));

      await expect(UserOrganizationService.create(validPayload)).rejects.toThrow("Fail");
  ...
  describe("getById", () => {
    it("should find by ObjectId", async () => {
      ...
      const res = await UserOrganizationService.getById(mockMappingId.toHexString());
    ...
    it("should find by reference lookup (Practitioner/ID)", async () => {
      ...
      const res = await UserOrganizationService.getById(mockUserId.toHexString());
  ...
  describe("update", () => {
    it("should return null if mapping not found", async () => {
        (UserOrganizationModel.findOne as jest.Mock).mockReturnValue(mockChain(null));
        const res = await UserOrganizationService.update(mockMappingId.toHexString(), validPayload);
        expect(res).toBeNull();
    });
    ...
    it("should send email if permissions changed", async () => {
        ...
        (UserModel.findOne as jest.Mock).mockReturnValue(mockChain({ email: "test@test.com", firstName: "Test" }));
        (OrganizationModel.findOne as jest.Mock).mockReturnValue(mockChain({ name: "Test Org" }));

        await UserOrganizationService.update(mockMappingId.toHexString(), validPayload);
  ...
  describe("Aggregated Lists (UserId / OrgId)", () => {
    it("listByUserId: returns details with billing if permitted", async () => {
        ...
        (UserOrganizationModel.find as jest.Mock).mockResolvedValue([mockDoc(mappingWithPerms)]);
        (OrganizationModel.findOne as jest.Mock).mockReturnValue(mockChain({ _id: mockOrgId }));

        const res = await UserOrganizationService.listByUserId(mockUserId.toHexString());
    ...
    it("listByUserId: hides billing if not permitted", async () => {
        const mappingNoPerms = {
            ...
            revokedPermissions: ["billing:view:any", "billing:edit:any", "billing:edit:limited"],
        };
        (UserOrganizationModel.find as jest.Mock).mockResolvedValue([mockDoc(mappingNoPerms)]);
        (OrganizationModel.findOne as jest.Mock).mockReturnValue(mockChain({ _id: mockOrgId }));
    ...
    it("listByOrganisationId: aggregates user details", async () => {
        (UserOrganizationModel.find as jest.Mock).mockResolvedValue([
            mockDoc({ ...validPayload, practitionerReference: mockUserId.toHexString() })
        ]);
        (UserModel.findOne as jest.Mock).mockResolvedValue({ firstName: "John", lastName: "Doe" });
        (UserProfileModel.findOne as jest.Mock).mockResolvedValue({ personalDetails: { profilePictureUrl: "http" } });
        (AvailabilityService.getCurrentStatus as jest.Mock).mockResolvedValue("AVAILABLE");
        (OccupancyModel.countDocuments as jest.Mock).mockResolvedValue(5);

        const res = await UserOrganizationService.listByOrganisationId(mockOrgId.toHexString());
  ...
  describe("Slot Management & Billing", () => {
    it("should handle Free Plan limit reached", async () => {
        ...
        await expect(UserOrganizationService.create(validPayload))
    ...
    it("should send email when free limit is just reached", async () => {
        ...
        const usageDoc = {
            _id: "u1",
            usersActiveCount: 5, freeUsersLimit: 5,
            appointmentsUsed: 0, freeAppointmentsLimit: 10,
            toolsUsed: 0, freeToolsLimit: 10,
            freeLimitReachedAt: null
        };
        (OrgUsageCounters.findOneAndUpdate as jest.Mock).mockResolvedValue(usageDoc);
        ...
        expect(OrgUsageNotifications.sendFreePlanLimitReachedEmail).toHaveBeenCalled();
    });
  });
  ...
  describe("Helpers coverage (Implicit)", () => {
    ...
    it("should throw error in createUserOrganizationMapping if no doc returned", async () => {
        ...
        await expect(UserOrganizationService.createUserOrganizationMapping(validPayload))
    ...
    it("should fail ensureSafeIdentifier with invalid chars", async () => {
        ...
        await expect(UserOrganizationService.upsert(badPayload))
            .rejects.toThrow("Invalid character in Identifier");
    });
  });
});