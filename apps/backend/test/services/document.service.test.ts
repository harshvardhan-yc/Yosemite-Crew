...
import { deleteFromS3, generatePresignedDownloadUrl } from 'src/middlewares/upload';
...
return jest.fn((str) => String(str).replaceAll(/[.*+?^${}()|[\]\\]/g, String.raw`\$&`));
});
...
describe('DocumentService', () => {
  ...
  describe('create & Internal Mappers (mapDocumentToDto, buildPersistableDocument)', () => {
    ...
    it('should throw if title is empty', async () => {
      await expect(
        DocumentService.create({ ...baseInput, title: '   ' }, { parentId: validObjectIdStr })
      ).rejects.toThrow(new DocumentServiceError('Document title is required.', 400));
    });

    it('should throw if attachments array is empty or missing', async () => {
      await expect(
        DocumentService.create({ ...baseInput, attachments: [] }, { parentId: validObjectIdStr })
      ).rejects.toThrow(new DocumentServiceError('At least one attachment is required.', 400));
    });

    it('should throw if companionId is invalid (ensureObjectId helper)', async () => {
      await expect(
        DocumentService.create({ ...baseInput, companionId: 'invalid' }, { parentId: validObjectIdStr })
    ...
    it('should throw if category is invalid', async () => {
      await expect(
        DocumentService.create({ ...baseInput, category: 'INVALID_CAT' }, { parentId: validObjectIdStr })
      ).rejects.toThrow(new DocumentServiceError('Invalid document category: INVALID_CAT', 400));
    });

    it('should throw if subcategory is invalid for the category', async () => {
      await expect(
        DocumentService.create({ ...baseInput, category: 'HEALTH', subcategory: 'INVALID_SUB' }, { parentId: validObjectIdStr })
      ).rejects.toThrow(new DocumentServiceError("Invalid subcategory 'INVALID_SUB' for category 'HEALTH'", 400));
    });
    ...
    it('should handle invalid string dates and empty string dates gracefully (ignore them)', async () => {
      ...
      expect(DocumentModel.create).toHaveBeenCalledWith(expect.objectContaining({ issueDate: null }));
  ...
  describe('deleteForParent', () => {
    it('should throw 404 if document not found or parent mismatch', async () => {
      mockExec.mockResolvedValue(null);
      await expect(DocumentService.deleteForParent(validObjectIdStr, validObjectIdStr))
        .rejects.toThrow(new DocumentServiceError('Document not found or not deletable.', 404));
    });
    ...
    it('should delete from S3 and DB on success', async () => {
      ...
      const result = await DocumentService.deleteForParent(validObjectIdStr, validObjectIdStr);
  ...
  describe('listForAppointmentParent & listForAppointmentPms', () => {
    ...
    it('listForAppointmentPms should query by appointmentId, companionId, and pmsVisible', async () => {
      ...
      await DocumentService.listForAppointmentPms({
        appointmentId: validObjectIdStr, companionId: validObjectIdStr
      });
      expect(DocumentModel.find).toHaveBeenCalledWith({
        companionId: new Types.ObjectId(validObjectIdStr),
        appointmentId: new Types.ObjectId(validObjectIdStr),
  ...
  describe('update', () => {
    it('should throw 404 if document not found', async () => {
      mockExec.mockResolvedValue(null);
      await expect(DocumentService.update(validObjectIdStr, {}, { parentId: validObjectIdStr }))
    ...
    it('should throw 403 if parent tries to update document they did not upload', async () => {
      const mockDoc = createMockDoc({ uploadedByParentId: new Types.ObjectId() }); // different ID
      mockExec.mockResolvedValue(mockDoc);

      await expect(DocumentService.update(validObjectIdStr, {}, { parentId: validObjectIdStr }))
        .rejects.toThrow(new DocumentServiceError('Parent is not allowed to update this document.', 403));
    });
    ...
    it('should throw 403 if PMS tries to update parent-uploaded document', async () => {
      ...
      await expect(DocumentService.update(validObjectIdStr, {}, { pmsUserId: 'pms1' }))
        .rejects.toThrow(new DocumentServiceError('PMS cannot update documents uploaded by parent.', 403));
    });

    it('should successfully apply updates and format properties', async () => {
      const parentId = new Types.ObjectId();
      const mockDoc = createMockDoc({ uploadedByParentId: parentId, category: 'HEALTH', subcategory: null });
      mockExec.mockResolvedValue(mockDoc);
    });

    it('should handle updates where issuingBusinessName is explicitly empty or subcategory is untouched', async () => {
      const parentId = new Types.ObjectId();
      const mockDoc = createMockDoc({ uploadedByParentId: parentId, category: 'HEALTH', subcategory: 'HOSPITAL_VISITS' });
      mockExec.mockResolvedValue(mockDoc);

      const updates = { issuingBusinessName: '', category: 'HEALTH' }; // Same category, triggers branch

      await DocumentService.update(validObjectIdStr, updates as any, { parentId });
    ...
    it('should ignore invalid issueDate formats during update', async () => {
      const parentId = new Types.ObjectId();
      const mockDoc = createMockDoc({ uploadedByParentId: parentId, issueDate: new Date('2026-01-01') });
      mockExec.mockResolvedValue(mockDoc);

      await DocumentService.update(validObjectIdStr, { issueDate: 'invalid-date' }, { parentId });

      // Kept the old valid date
      expect(mockDoc.issueDate.toISOString()).toBe(new Date('2026-01-01').toISOString());
    });
  });
  ...
  describe('getAllAttachmentUrls', () => {
    it('should throw 404 if document not found or has no attachments', async () => {
      ...
      await expect(DocumentService.getAllAttachmentUrls(validObjectIdStr))
    ...
    it('should return presigned URLs mapped correctly', async () => {
      const mockDoc = createMockDoc({
        attachments: [{ key: 'k1', mimeType: 'img/png' }, { key: 'k2', mimeType: 'app/pdf' }]
      });
      mockExec.mockResolvedValue(mockDoc);
      ...
      const result = await DocumentService.getAllAttachmentUrls(validObjectIdStr);

      expect(result).toHaveLength(2);
      expect(result[0]).toEqual({ url: 'url1', mimeType: 'img/png', key: 'k1' });
      expect(result[1]).toEqual({ url: 'url2', mimeType: 'app/pdf', key: 'k2' });
    });
  });

  describe('searchByTitleForParent', () => {
    it('should throw 400 if title is missing or not a string', async () => {
      await expect(DocumentService.searchByTitleForParent({ companionId: validObjectIdStr, title: '' }))
        .rejects.toThrow(new DocumentServiceError('Search title is required.', 400));

      await expect(DocumentService.searchByTitleForParent({ companionId: validObjectIdStr, title: 123 as any }))
        .rejects.toThrow(new DocumentServiceError('Search title is required.', 400));
    });
    ...
    it('should query by regex and return documents', async () => {
      ...
      const result = await DocumentService.searchByTitleForParent({ companionId: validObjectIdStr, title: ' test ' });
