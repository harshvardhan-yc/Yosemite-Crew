...
describe('ParentService', () => {
  ...
  describe('create', () => {
    ...
    it('should create a parent for mobile user', async () => {
      (AuthUserMobileService.getAuthUserMobileIdByProviderId as any).mockResolvedValue(mobileId);
      ...
      const result = await ParentService.create(inputDto, { source: 'mobile', authUserId });

      expect(AuthUserMobileService.getAuthUserMobileIdByProviderId).toHaveBeenCalledWith(authUserId);
      expect(ParentModel.create).toHaveBeenCalledWith(expect.objectContaining({
        firstName: 'John',
        linkedUserId: mobileId,
        createdFrom: 'mobile'
      }));
      expect(UploadMiddleware.moveFile).toHaveBeenCalledWith('temp-url', 'key');
      expect(AuthUserMobileService.linkParent).toHaveBeenCalledWith(authUserId, createdDoc._id.toString());
      expect(result.isProfileComplete).toBe(true);
    });

    it('should create a parent for pms (no auth user)', async () => {
      const createdDoc = mockDoc({ firstName: 'John', email: 'john@example.com' });
      (ParentModel.create as any).mockResolvedValue(createdDoc);
    ...
    it('should throw if mobile user missing authUserId', async () => {
      await expect(ParentService.create(baseDto, { source: 'mobile' }))
    ...
    it('should throw if parent already exists for user', async () => {
      ...
      await expect(ParentService.create(baseDto, { source: 'mobile', authUserId }))
    ...
    it('should throw if email is missing in DTO', async () => {
       ...
       await expect(ParentService.create(invalidDto, { source: 'pms' }))
         .rejects.toThrow('Parent email is required');
    });

    it('should handle image upload failure gracefully', async () => {
        const createdDoc = mockDoc({ firstName: 'John', profileImageUrl: 'temp-url' });
        (ParentModel.create as any).mockResolvedValue(createdDoc);
        (UploadMiddleware.moveFile as any).mockRejectedValue(new Error('S3 Error'));
  ...
  describe('get', () => {
    it('should return parent by ID', async () => {
      ...
      expect(ParentModel.findOne).toHaveBeenCalledWith({ _id: new Types.ObjectId(parentId) });
    });
    ...
    it('should enforce ownership for mobile users', async () => {
      ...
      expect(ParentModel.findOne).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should throw on invalid ID', async () => {
      await expect(ParentService.get('invalid')).rejects.toThrow('Invalid identifier');
    });
  });
  ...
  describe('update', () => {
    ...
    it('should enforce ownership for mobile update', async () => {
        ...
        await ParentService.update(parentId, updateDto, { source: 'mobile', authUserId });
  ...
  describe('delete', () => {
      ...
      it('should enforce ownership for mobile delete', async () => {
          ...
          expect(ParentModel.findOneAndDelete).toHaveBeenCalledWith(expect.objectContaining({
              linkedUserId: mobileId
          }));
      });

      it('should throw if mobile auth missing on delete', async () => {
          await expect(ParentService.delete(parentId, { source: 'mobile' }))
            .rejects.toThrow('Authenticated user ID required');
      });
  ...
  describe('Helpers', () => {
      it('findByLinkedUserId: success', async () => {
          ...
          expect(ParentModel.findOne).toHaveBeenCalledWith({ linkedUserId: mobileId });
      });

      it('findByLinkedUserId: throw if invalid input', async () => {
          await expect(ParentService.findByLinkedUserId('')).rejects.toThrow('Invalid AuthUser ID');
      });
      ...
      it('getByName: should search regex', async () => {
          (ParentModel.find as any).mockResolvedValue([mockDoc({ firstName: 'John' })]);
          const res = await ParentService.getByName('John');
          expect(res.responses).toHaveLength(1);
          expect(ParentModel.find).toHaveBeenCalledWith(expect.objectContaining({ $or: expect.any(Array) }));
      });

      it('getByName: throw if empty', async () => {
          await expect(ParentService.getByName('')).rejects.toThrow('Name is required');
      });
  });
});