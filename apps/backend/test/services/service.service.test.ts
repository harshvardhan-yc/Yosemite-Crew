...
const mockDoc = (data: any) => {
  ...
  const organisationId = data.organisationId instanceof Types.ObjectId
    ? data.organisationId
    : (data.organisationId ? new Types.ObjectId(data.organisationId) : undefined);
...
describe('ServiceService', () => {
  ...
  describe('CRUD', () => {
    it('create: should create a service', async () => {
      ...
      expect(ServiceModel.create).toHaveBeenCalledWith(expect.objectContaining({
        name: 'Vaccination',
    ...
    it('create: should handle null optional fields', async () => {
        const dto = { name: 'Simple', organisationId: orgId, cost: 50, durationMinutes: 10, isActive: true };
        const createdDoc = mockDoc(dto);
        (ServiceModel.create as any).mockResolvedValue(createdDoc);
    ...
    it('update: should handle clearing optional fields', async () => {
        const doc = mockDoc({ name: 'Orig', organisationId: orgId, observationToolId: toolId });
        (ServiceModel.findById as any).mockResolvedValue(doc);

        // Update with null/falsey values
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await ServiceService.update(serviceId, { observationToolId: null } as any);
    ...
    it('update: should throw if not found', async () => {
        (ServiceModel.findById as any).mockResolvedValue(null);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await expect(ServiceService.update(serviceId, {} as any)).rejects.toThrow('Service not found');
    });
  ...
  describe('Search & Listing', () => {
      ...
      it('listOrganisationsProvidingService: should aggregate distinct orgs', async () => {
          ...
          (OrganizationModel.find as any).mockReturnValue(mockMongooseChain([
              { _id: new Types.ObjectId(orgId), name: 'Org 1' }
          ]));

          const res = await ServiceService.listOrganisationsProvidingService('vaccine');
      ...
      it('listOrganisationsProvidingService: returns empty if no services found', async () => {
          (ServiceModel.find as any).mockReturnValue(mockMongooseChain([]));
          const res = await ServiceService.listOrganisationsProvidingService('unknown');
          expect(res).toEqual([]);
      });
  });
  ...
  describe('getBookableSlotsService', () => {
      it('should throw if service or speciality not found', async () => {
          (ServiceModel.findById as any).mockResolvedValue(null);
          await expect(ServiceService.getBookableSlotsService(serviceId, orgId, new Date()))
      ...
      it('should return empty if no vets in speciality', async () => {
          (ServiceModel.findById as any).mockResolvedValue({ specialityId });
          (SpecialityModel.findById as any).mockResolvedValue({ memberUserIds: [] });

          const res = await ServiceService.getBookableSlotsService(serviceId, orgId, new Date());
      ...
      it('should aggregate slots from multiple vets and deduplicate', async () => {
          ...
          (ServiceModel.findById as any).mockResolvedValue({ specialityId, durationMinutes: 15 });
          (SpecialityModel.findById as any).mockResolvedValue({ memberUserIds: [vet1, vet2] });

          // Mock AvailabilityService to return slots
          // vet1 has slot at 10:00
          // vet2 has slot at 10:00 (overlap) and 11:00
          (AvailabilityService.getBookableSlotsForDate as any).mockImplementation((_oid: any, vid: any) => {
              if (vid === vet1) return { windows: [{ startTime: '10:00', endTime: '10:15' }] };
              if (vid === vet2) return { windows: [
          ...
          const res = await ServiceService.getBookableSlotsService(serviceId, orgId, date);
      ...
      it('should filter past slots if querying for today', async () => {
          ...
          (ServiceModel.findById as any).mockResolvedValue({ specialityId, durationMinutes: 15 });
          (SpecialityModel.findById as any).mockResolvedValue({ memberUserIds: ['v1'] });
          (AvailabilityService.getBookableSlotsForDate as any).mockResolvedValue({
              windows: [pastSlot, futureSlot]
          });

          const res = await ServiceService.getBookableSlotsService(serviceId, orgId, today);
  ...
  describe('listOrganisationsProvidingServiceNearby', () => {
      it('should aggregate orgs, specialities, and services', async () => {
          ...
          (OrganizationModel.find as any).mockReturnValue(mockMongooseChain([orgDoc]));
          ...
          (SpecialityModel.find as any).mockReturnValue(mockMongooseChain([specDoc]));
          ...
          const res = await ServiceService.listOrganisationsProvidingServiceNearby('Checkup', 0, 0, 'City');
      ...
      it('should return empty if no matching services found', async () => {
          (ServiceModel.find as any).mockReturnValue(mockMongooseChain([]));
          const res = await ServiceService.listOrganisationsProvidingServiceNearby('Unknown', 10, 10);
          expect(res).toEqual([]);
      });
  });
  ...
  describe('Utils', () => {
      it('ensureObjectId: should throw on invalid ID', async () => {
          await expect(ServiceService.getById('invalid-id')).rejects.toThrow('Invalid serviceId');
      });
  });
});