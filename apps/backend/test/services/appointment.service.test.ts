...
const mockDoc = (data: any = {}) => {
  const _id = data._id
    ...
    : new Types.ObjectId(VALID_APP_ID);
...
describe("AppointmentService", () => {
  ...
  describe("createRequestedFromMobile", () => {
    ...
    it("should throw if validation fails", async () => {
      ...
      await expect(
        AppointmentService.createRequestedFromMobile(validDto),
    ...
    it("should release usage if db create fails", async () => {
      ...
      (AppointmentModel.create as jest.Mock).mockRejectedValue(
        new AppointmentServiceError("DB Fail", 500),
      );
  ...
  describe("createAppointmentFromPms", () => {
    ...
    it("should throw on occupancy conflict", async () => {
      ...
      await expect(
        AppointmentService.createAppointmentFromPms(validPmsDto, false),
  ...
  describe("approveRequestedFromPms", () => {
    const approvalDto: any = {
      ...
      participant: [
        {
          actor: { reference: "Practitioner/l1", display: "Dr. Lead" },
    ...
    it("should throw if missing lead vet", async () => {
      ...
      await expect(
        AppointmentService.approveRequestedFromPms(VALID_APP_ID, invalid),
    ...
    it("should throw if appointment not found", async () => {
      ...
      await expect(
        AppointmentService.approveRequestedFromPms(VALID_APP_ID, approvalDto),
  ...
  describe("cancelAppointmentFromParent", () => {
    ...
    it("should throw if not owner", async () => {
      ...
      await expect(
        AppointmentService.cancelAppointmentFromParent(VALID_APP_ID, "p2", "R"),
  ...
  describe("rejectRequestedAppointment", () => {
    ...
    it("should throw if not REQUESTED", async () => {
      ...
      (AppointmentModel.findById as jest.Mock).mockResolvedValue(appDoc);
      await expect(
        AppointmentService.rejectRequestedAppointment(VALID_APP_ID),
      ).rejects.toThrow("Only REQUESTED");
    });
  });
  ...
  describe("updateAppointmentPMS", () => {
    ...
    it("should throw if conflict on new slot", async () => {
      ...
      (AppointmentModel.findOne as jest.Mock).mockResolvedValue(appDoc);
      (OccupancyModel.findOne as jest.Mock).mockReturnValue(
        mockChain({ _id: "conflict" }),
      );

      await expect(
        AppointmentService.updateAppointmentPMS(VALID_APP_ID, updateDto),
      ).rejects.toThrow("Selected vet is not available");
    });
  });
  ...
  describe("checkInAppointment", () => {
    ...
    it("should throw if wrong status", async () => {
      ...
      (AppointmentModel.findById as jest.Mock).mockResolvedValue(appDoc);
      await expect(
        AppointmentService.checkInAppointment(VALID_APP_ID),
      ).rejects.toThrow("Only upcoming");
    });
  });
  ...
  describe("checkInAppointmentParent", () => {
    ...
    it("should throw if not owner", async () => {
      ...
      (AppointmentModel.findById as jest.Mock).mockResolvedValue(appDoc);
      await expect(
        AppointmentService.checkInAppointmentParent(VALID_APP_ID, "p2"),
      ).rejects.toThrow("Not your appointment");
    });
  });
  ...
  describe("rescheduleFromParent", () => {
    const changes = {
      ...
      endTime: new Date(Date.now() + 3600).toISOString(),
    };
    ...
    it("should throw if invalid times", async () => {
      await expect(
        AppointmentService.rescheduleFromParent(VALID_APP_ID, "p1", {
          startTime: "invalid",
          endTime: "invalid",
        }),
  ...
  describe("Queries", () => {
    ...
    it("getAppointmentsForLead", async () => {
      ...
      expect(AppointmentModel.find).toHaveBeenCalledWith(
        expect.objectContaining({ "lead.id": "l1" }),
      );
    });
  ...
  describe("Helpers (Coverage)", () => {
    it("sendAppointmentAssignmentEmails handles errors gracefully", async () => {
      ...
      const dto = {
        ...
        participant: [
          {
            actor: { reference: "Practitioner/l1" },
            type: [{ coding: [{ code: "PPRF" }] }],
          },
        ],
      };
