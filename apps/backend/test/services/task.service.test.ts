...
describe('TaskService', () => {
  ...
  describe('Internal Helpers (Medication & Recurrence)', () => {
    ...
    it('should sanitize medication input (valid)', async () => {
      ...
      await TaskService.createCustom({
        category: 'Health', name: 'Meds', createdBy: 'u1', assignedTo: 'u1', dueAt: mockDate, audience: 'EMPLOYEE_TASK',
    ...
    it('should return undefined for empty medication object', async () => {
        (TaskModel.create as jest.Mock).mockResolvedValue(mockDoc({}));
        await TaskService.createCustom({
            category: 'C', name: 'N', createdBy: 'u1', assignedTo: 'u1', dueAt: mockDate, audience: 'EMPLOYEE_TASK',
    ...
    it('should build ONCE recurrence correctly', async () => {
        ...
        expect(TaskModel.create).toHaveBeenCalledWith(expect.objectContaining({
            recurrence: expect.objectContaining({ type: 'ONCE', isMaster: false })
        }));
    });
    ...
    it('should build RECURRING recurrence correctly', async () => {
        ...
        await TaskService.createCustom({
            category: 'C', name: 'N', createdBy: 'u1', assignedTo: 'u1', dueAt: mockDate, audience: 'EMPLOYEE_TASK',
            recurrence: { type: 'DAILY', cronExpression: '* * *', endDate: mockDate }
        });
        expect(TaskModel.create).toHaveBeenCalledWith(expect.objectContaining({
            recurrence: expect.objectContaining({ type: 'DAILY', isMaster: true, endDate: mockDate })
        }));
    });
  });
  ...
  describe('createCustom', () => {
    it('should create a custom task', async () => {
      (TaskModel.create as jest.Mock).mockResolvedValue(mockDoc({ _id: 't1', audience: 'EMPLOYEE_TASK' }));

      const result = await TaskService.createCustom({
        category: 'Misc', name: 'Custom Task',
        createdBy: 'u1', assignedTo: 'u1', dueAt: mockDate, audience: 'EMPLOYEE_TASK'
      });

      expect(result).toBeDefined();
      expect(TaskModel.create).toHaveBeenCalledWith(expect.objectContaining({ source: 'CUSTOM' }));
    });

    it('should throw if name/category missing', async () => {
      await expect(TaskService.createCustom({ name: '' } as any)).rejects.toThrow('category and name are required');
    });
  });
  ...
  describe('sendTaskAssignmentEmail (Private function triggered via create)', () => {
    it('should send email if audience is EMPLOYEE_TASK', async () => {
      ...
      const taskDoc = mockDoc({
        _id: 't1', audience: 'EMPLOYEE_TASK', name: 'Task',
        assignedTo: 'u2', createdBy: 'u1', dueAt: mockDate
      });
      (TaskModel.create as jest.Mock).mockResolvedValue(taskDoc);

      // Setup User mocks for email logic
      (UserModel.findOne as jest.Mock)
        .mockReturnValueOnce(mockChain({ email: 'u2@test.com', firstName: 'User', lastName: 'Two' })) // Assignee
        .mockReturnValueOnce(mockChain({ firstName: 'User', lastName: 'One' })); // Assigner

      await TaskService.createCustom({
        category: 'C', name: 'T', createdBy: 'u1', assignedTo: 'u2', dueAt: mockDate, audience: 'EMPLOYEE_TASK'
      });

      // We need to wait a tick because sendTaskAssignmentEmail is called without await (void)
      await new Promise(process.nextTick);

      expect(sendEmailTemplate).toHaveBeenCalledWith(expect.objectContaining({
        to: 'u2@test.com',
    ...
    it('should NOT send email if audience is PARENT_TASK', async () => {
      const taskDoc = mockDoc({ _id: 't1', audience: 'PARENT_TASK', assignedTo: 'u2', createdBy: 'u1', companionId: 'c1', dueAt: mockDate });
      (TaskModel.create as jest.Mock).mockResolvedValue(taskDoc);

      await TaskService.createCustom({
        category: 'C', name: 'T', createdBy: 'u1', assignedTo: 'u2', dueAt: mockDate, audience: 'PARENT_TASK', companionId: 'c1'
      });
    ...
    it('should catch errors during email sending', async () => {
       const taskDoc = mockDoc({ _id: 't1', audience: 'EMPLOYEE_TASK', assignedTo: 'u2', dueAt: mockDate });
       (TaskModel.create as jest.Mock).mockResolvedValue(taskDoc);
       (UserModel.findOne as jest.Mock).mockReturnValue(mockChain({ email: 'u2@test.com' }));
       ...
       expect(logger.error).toHaveBeenCalledWith(expect.stringContaining('Failed to send task assignment email'), expect.anything());
    });
  });
  ...
  describe('updateTask', () => {
    it('should allow creator to update and reassign', async () => {
      ...
      (TaskModel.findById as jest.Mock).mockReturnValue({ exec: jest.fn().mockResolvedValue(task) });

      await TaskService.updateTask('t1', { assignedTo: 'u2', name: 'New Name' }, 'u1');
    ...
    it('should forbid non-creator from reassigning', async () => {
      ...
      await expect(TaskService.updateTask('t1', { assignedTo: 'u3' }, 'u2'))
    ...
    it('should forbid random user from updating', async () => {
        ...
        await expect(TaskService.updateTask('t1', { name: 'Hack' }, 'u3'))
          .rejects.toThrow('Not allowed to update this task');
    });

    it('should handle complex updates (medication, reminder, recurrence)', async () => {
        const task = mockDoc({ createdBy: 'u1', assignedTo: 'u1', recurrence: { type: 'ONCE' } });
        (TaskModel.findById as jest.Mock).mockReturnValue({ exec: jest.fn().mockResolvedValue(task) });
    ...
    it('should handle recurrence updates when existing recurrence is null', async () => {
        const task = mockDoc({ createdBy: 'u1', assignedTo: 'u1', recurrence: undefined });
        (TaskModel.findById as jest.Mock).mockReturnValue({ exec: jest.fn().mockResolvedValue(task) });

        await TaskService.updateTask('t1', { recurrence: { type: 'DAILY' } }, 'u1');
        expect(task.recurrence).toBeDefined();
    });

    it('should handle recurrence updates clearing recurrence', async () => {
        const task = mockDoc({ createdBy: 'u1', assignedTo: 'u1', recurrence: {} });
        (TaskModel.findById as jest.Mock).mockReturnValue({ exec: jest.fn().mockResolvedValue(task) });
  ...
  describe('changeStatus', () => {
    ...
    it('should change status to COMPLETED and create Completion record', async () => {
        const task = mockDoc({ _id: 't1', createdBy: 'u1', status: 'IN_PROGRESS', companionId: 'c1' });
        (TaskModel.findById as jest.Mock).mockReturnValue({ exec: jest.fn().mockResolvedValue(task) });
        (TaskCompletionModel.create as jest.Mock).mockResolvedValue({ _id: 'comp1' });

        const result = await TaskService.changeStatus('t1', 'COMPLETED', 'u1', { filledBy: 'u1', answers: { q: 1 } });

        expect(task.status).toBe('COMPLETED');
        expect(task.completedAt).toBeDefined();
        expect(TaskCompletionModel.create).toHaveBeenCalledWith(expect.objectContaining({ taskId: 't1' }));
    ...
    it('should throw if task already finished', async () => {
        ...
        (TaskModel.findById as jest.Mock).mockReturnValue({ exec: jest.fn().mockResolvedValue(task) });
        await expect(TaskService.changeStatus('t1', 'PENDING', 'u1')).rejects.toThrow('Task already finished');
    });
  ...
  describe('listForParent', () => {
      it('should list tasks with correct filters', async () => {
          ...
          await TaskService.listForParent({
              parentId: 'p1', companionId: 'c1', fromDueAt: mockDate, status: ['PENDING']
          });
  ...
  describe('listForCompanion', () => {
      it('should list tasks for companion', async () => {
          (TaskModel.find as jest.Mock).mockReturnValue(mockChain([]));
          await TaskService.listForCompanion({ companionId: 'c1', audience: 'PARENT_TASK' });
          expect(TaskModel.find).toHaveBeenCalledWith(expect.objectContaining({
              companionId: 'c1',
              audience: 'PARENT_TASK'
          }));
  ...
  describe('linkToAppointment', () => {
      it('should update appointmentId', async () => {
          ...
          await TaskService.linkToAppointment({ taskId: 't1', appointmentId: 'a1' });
          expect(task.appointmentId).toBe('a1');
          expect(task.save).toHaveBeenCalled();
      });

      it('should throw if task not found', async () => {
        (TaskModel.findById as jest.Mock).mockReturnValue({ exec: jest.fn().mockResolvedValue(null) });
        await expect(TaskService.linkToAppointment({ taskId: 't1', appointmentId: 'a1' })).rejects.toThrow('not found');
      });
  });
});