...
import { CoParentInviteService, CoParentInviteServiceError } from '../../src/services/coparentInvite.service';
...
describe('CoParentInviteService', () => {
  ...
  describe('sendInvite', () => {
    it('should throw if email is missing', async () => {
      await expect(
        CoParentInviteService.sendInvite({ email: '', companionId: validCompanionId, invitedByParentId: validParentId })
      ).rejects.toThrow(new CoParentInviteServiceError('Email is required.', 400));
    });

    it('should throw if companionId is invalid', async () => {
      await expect(
        CoParentInviteService.sendInvite({ email: 'test@test.com', companionId: 'invalid', invitedByParentId: validParentId })
      ).rejects.toThrow(new CoParentInviteServiceError('Invalid companionId.', 400));
    });

    it('should throw if invitedByParentId is invalid', async () => {
      await expect(
        CoParentInviteService.sendInvite({ email: 'test@test.com', companionId: validCompanionId, invitedByParentId: 'invalid' })
      ).rejects.toThrow(new CoParentInviteServiceError('Invalid invitedByParentId.', 400));
    });
  ...
  describe('validateInvite', () => {
    it('should throw if token is missing or not a string', async () => {
      ...
      await expect(CoParentInviteService.validateInvite(null as any)).rejects.toThrow('Invite token is required.');
    });
    ...
    it('should throw if invite is consumed', async () => {
      (CoParentInviteModel.findOne as jest.Mock).mockResolvedValue({ consumed: true });
    ...
    it('should throw if inviter parent not found', async () => {
      (CoParentInviteModel.findOne as jest.Mock).mockResolvedValue({
        consumed: false, expiresAt: new Date(Date.now() + 10000), invitedByParentId: validParentId
      });
      (ParentModel.findById as jest.Mock).mockReturnValue(createMockQuery(null));
    ...
    it('should throw if companion not found', async () => {
      (CoParentInviteModel.findOne as jest.Mock).mockResolvedValue({
        consumed: false, expiresAt: new Date(Date.now() + 10000), invitedByParentId: validParentId, companionId: validCompanionId
      });
      (ParentModel.findById as jest.Mock).mockReturnValue(createMockQuery({ _id: validParentId }));
      (CompanionModel.findById as jest.Mock).mockReturnValue(createMockQuery(null));
    ...
    it('should return fully mapped data when optionals are present', async () => {
      (CoParentInviteModel.findOne as jest.Mock).mockResolvedValue({
        _id: 'invite_id', email: 'test@test.com', inviteeName: 'Invitee',
        consumed: false, expiresAt: new Date(Date.now() + 10000),
        invitedByParentId: validParentId, companionId: validCompanionId
      });
      (ParentModel.findById as jest.Mock).mockReturnValue(createMockQuery({
        _id: validParentId, firstName: 'John', lastName: 'Doe', profileImageUrl: 'url'
      }));
      (CompanionModel.findById as jest.Mock).mockReturnValue(createMockQuery({
        _id: validCompanionId, name: 'Child', photoUrl: 'photo'
      }));
    ...
    it('should return mapped data falling back to null for optionals', async () => {
      (CoParentInviteModel.findOne as jest.Mock).mockResolvedValue({
        _id: 'invite_id', email: 'test@test.com',
        consumed: false, expiresAt: new Date(Date.now() + 10000),
        invitedByParentId: validParentId, companionId: validCompanionId
      });
      (ParentModel.findById as jest.Mock).mockReturnValue(createMockQuery({
        _id: validParentId, firstName: 'John' // Missing last name & image
      }));
      (CompanionModel.findById as jest.Mock).mockReturnValue(createMockQuery({
        _id: validCompanionId, name: 'Child' // Missing photoUrl
      }));
  ...
  describe('acceptInvite', () => {
    ...
    it('should throw if token is missing', async () => {
      await expect(CoParentInviteService.acceptInvite('', 'user1')).rejects.toThrow('Invite token is required.');
    });
    ...
    it('should throw if inviteDoc is not found after validation', async () => {
      validateSpy.mockResolvedValue({ id: 'inv_id', companion: { id: 'c1' }, invitedBy: { id: 'p1' } } as any);
      (CoParentInviteModel.findById as jest.Mock).mockResolvedValue(null);

      await expect(CoParentInviteService.acceptInvite('token', 'user1')).rejects.toThrow('Invalid invite');
    });
    ...
    it('should throw if parent profile not found for accepting user', async () => {
      ...
      await expect(CoParentInviteService.acceptInvite('token', 'user1')).rejects.toThrow('Parent profile not found');
    });

    it('should throw if an existing link already exists', async () => {
      validateSpy.mockResolvedValue({ id: 'inv_id', companion: { id: 'c1' }, invitedBy: { id: 'p1' } } as any);
      (CoParentInviteModel.findById as jest.Mock).mockResolvedValue({});
      (ParentService.findByLinkedUserId as jest.Mock).mockResolvedValue({ _id: 'p2' });
      (ParentCompanionModel.findOne as jest.Mock).mockResolvedValue({ _id: 'link1' }); // Link exists

      await expect(CoParentInviteService.acceptInvite('token', 'user1')).rejects.toThrow('You are already linked to this companion.');
    });

    it('should create link, mark as consumed, save and return success', async () => {
      validateSpy.mockResolvedValue({ id: 'inv_id', companion: { id: validCompanionId }, invitedBy: { id: validParentId } } as any);
  ...
  describe('declineInvite', () => {
    ...
    it('should throw if already consumed', async () => {
      (CoParentInviteModel.findOne as jest.Mock).mockResolvedValue({ consumed: true });
      await expect(CoParentInviteService.declineInvite('token')).rejects.toThrow('This invite has already been used.');
    });

    it('should throw if expired', async () => {
      (CoParentInviteModel.findOne as jest.Mock).mockResolvedValue({ consumed: false, expiresAt: new Date(Date.now() - 1000) });
      await expect(CoParentInviteService.declineInvite('token')).rejects.toThrow('This invite has expired.');
    });
    ...
    it('should mark consumed and return success', async () => {
      ...
      (CoParentInviteModel.findOne as jest.Mock).mockResolvedValue({
        consumed: false, expiresAt: new Date(Date.now() + 10000),
        email: 't@t.com', companionId: 'c1', invitedByParentId: 'p1',
        save: mockSave
      });
  ...
  describe('getPendingInvitesForEmail', () => {
    ...
    it('should return empty array if no invites found', async () => {
      (CoParentInviteModel.find as jest.Mock).mockReturnValue({ lean: jest.fn().mockResolvedValue([]) });
      const res = await CoParentInviteService.getPendingInvitesForEmail('test@test.com');
      expect(res.pendingInvites).toEqual([]);
    });

    it('should skip invite if inviter or companion is not found and map valid ones', async () => {
      const mockInvites = [
        { inviteToken: 't1', email: 'test@test.com', invitedByParentId: 'p1', companionId: 'c1' }, // Missing inviter
        { inviteToken: 't2', email: 'test@test.com', invitedByParentId: 'p2', companionId: 'c2' }, // Missing companion
        { inviteToken: 't3', email: 'test@test.com', invitedByParentId: 'p3', companionId: 'c3', inviteeName: 'Test Name' }, // Valid
      ];

      (CoParentInviteModel.find as jest.Mock).mockReturnValue({ lean: jest.fn().mockResolvedValue(mockInvites) });
