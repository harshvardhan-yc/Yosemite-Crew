...
describe('InvoiceService', () => {
  ...
  describe('Creation', () => {
    describe('createDraftForAppointment', () => {
      ...
      it('should throw if appointment not found', async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await expect(InvoiceService.createDraftForAppointment({ appointmentId } as any))
      ...
      it('should calculate discounts correctly', async () => {
         ...
         const input = {
             ...
             companionId: 'comp',
             currency: 'USD',
             items: [{ description: 'Item', quantity: 2, unitPrice: 50, discountPercent: 10 }] // Total 100, 10% off -> 90
         };
    ...
    describe('createExtraInvoiceForAppointment', () => {
      ...
      it('should throw if appointment missing', async () => {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          await expect(InvoiceService.createExtraInvoiceForAppointment({ appointmentId } as any))
            .rejects.toThrow('Appointment not found');
      });
    });
  });
  ...
  describe('Retrieval', () => {
    ...
    describe('getById', () => {
        it('should return invoice and organization details', async () => {
            ...
            (OrganizationModel.findById as any).mockResolvedValue({
                name: 'Org', googlePlacesId: 'pid', address: 'addr', imageURL: 'img'
            });
        ...
        it('should throw if invoice not found', async () => {
            ...
            await expect(InvoiceService.getById(validId)).rejects.toThrow('Invoice not found');
        });

        it('should validate ID format', async () => {
            await expect(InvoiceService.getById('invalid')).rejects.toThrow('Invalid invoiceId');
        });
    });
    ...
    const testListMethod = async (methodName: 'listForOrganisation' | 'listForParent' | 'listForCompanion' | 'getByAppointmentId', filterKey: string) => {
        ...
        expect(InvoiceModel.find).toHaveBeenCalledWith(expect.objectContaining({ [filterKey]: validId }));
    };

    describe('List Methods', () => {
        it('listForOrganisation', () => testListMethod('listForOrganisation', 'organisationId'));
        it('listForParent', () => testListMethod('listForParent', 'parentId'));
        it('listForCompanion', () => testListMethod('listForCompanion', 'companionId'));
        it('getByAppointmentId', () => testListMethod('getByAppointmentId', 'appointmentId'));
    });
  });
  ...
  describe('Status Management', () => {
      const doc = mockInvoiceDoc({ _id: new Types.ObjectId(validId), status: 'AWAITING_PAYMENT' });
      ...
      describe('attachStripeDetails', () => {
          it('should update and return domain object', async () => {
              ...
              const res = await InvoiceService.attachStripeDetails(validId, { stripeInvoiceId: 'inv_1' });
          ...
          it('should throw if not found', async () => {
              ...
              await expect(InvoiceService.attachStripeDetails(validId, {})).rejects.toThrow('Invoice not found');
          });
      });
      ...
      describe('markPaid / markFailed / markRefunded', () => {
          it('markPaid updates status', async () => {
             ...
             expect(InvoiceModel.findByIdAndUpdate).toHaveBeenCalledWith(expect.anything(), { $set: { status: 'PAID' } }, expect.anything());
          });

          it('markFailed updates status', async () => {
             (InvoiceModel.findByIdAndUpdate as any).mockResolvedValue(doc);
             await InvoiceService.markFailed(validId);
             expect(InvoiceModel.findByIdAndUpdate).toHaveBeenCalledWith(expect.anything(), { $set: { status: 'FAILED' } }, expect.anything());
          });

          it('markRefunded updates status', async () => {
             (InvoiceModel.findByIdAndUpdate as any).mockResolvedValue(doc);
             await InvoiceService.markRefunded(validId);
             expect(InvoiceModel.findByIdAndUpdate).toHaveBeenCalledWith(expect.anything(), { $set: { status: 'REFUNDED' } }, expect.anything());
          });
      });
      ...
      describe('updateStatus', () => {
          ...
          it('should throw if missing', async () => {
              (InvoiceModel.findById as any).mockResolvedValue(null);
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              await expect(InvoiceService.updateStatus(validId, 'PAID' as any)).rejects.toThrow('Invoice not found');
          });
      });
  });
  ...
  describe('Complex Logic', () => {
     ...
     describe('addItemsToInvoice', () => {
         it('should add items and recalculate totals', async () => {
             ...
             const newItems = [{ id: '2', name: 'New', quantity: 2, unitPrice: 50, total: 100 }];
             // eslint-disable-next-line @typescript-eslint/no-explicit-any
         ...
         it('should throw if invoice is PAID', async () => {
             ...
             (InvoiceModel.findById as any).mockResolvedValue(doc);
             await expect(InvoiceService.addItemsToInvoice(validId, [])).rejects.toThrow('Cannot modify a paid invoice');
         });
     });

     describe('addChargesToAppointment', () => {
         it('Scenario A: Open invoice exists -> append', async () => {
             const mockFind = jest.spyOn(InvoiceService, 'findOpenInvoiceForAppointment');
             ...
             const res = await InvoiceService.addChargesToAppointment(appointmentId, [], 'USD');
         ...
         it('Scenario B: No open invoice -> create extra', async () => {
             const mockFind = jest.spyOn(InvoiceService, 'findOpenInvoiceForAppointment');
             const mockCreate = jest.spyOn(InvoiceService, 'createExtraInvoiceForAppointment');
     ...
     describe('handleAppointmentCancellation', () => {
         it('should return NO_INVOICE if none exists', async () => {
             (InvoiceModel.findOne as any).mockResolvedValue(null);
             const res = await InvoiceService.handleAppointmentCancellation(appointmentId, 'reason');
             expect(res.action).toBe('NO_INVOICE');
         });

         it('should return ALREADY_HANDLED if status is CANCELLED', async () => {
             (InvoiceModel.findOne as any).mockResolvedValue({ status: 'CANCELLED' });
             const res = await InvoiceService.handleAppointmentCancellation(appointmentId, 'reason');
             expect(res.action).toBe('ALREADY_HANDLED');
         });

         it('should cancel unpaid invoice', async () => {
             const doc = mockInvoiceDoc({ status: 'AWAITING_PAYMENT', metadata: {} });
             (InvoiceModel.findOne as any).mockResolvedValue(doc);
         ...
         it('should throw if paid but no stripe ID', async () => {
             const doc = mockInvoiceDoc({ status: 'PAID', stripePaymentIntentId: null });
             (InvoiceModel.findOne as any).mockResolvedValue(doc);

             await expect(InvoiceService.handleAppointmentCancellation(appointmentId, 'reason'))
