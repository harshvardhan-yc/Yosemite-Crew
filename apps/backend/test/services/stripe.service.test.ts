...
describe("StripeService", () => {
  ...
  beforeEach(() => {
    ...
    stripeInstance = {
      ...
      checkout: {
        sessions: {
          create: jest
            .fn()
            .mockResolvedValue({ id: "cs_1", url: "http://checkout" }),
          retrieve: jest
            .fn()
            .mockResolvedValue({ payment_status: "paid", amount_total: 5000 }),
        },
      },
      billingPortal: {
        sessions: {
      ...
      paymentIntents: {
        create: jest
          .fn()
          .mockResolvedValue({ id: "pi_1", client_secret: "sec_1" }),
        retrieve: jest.fn().mockResolvedValue({
          id: "pi_existing",
      ...
      refunds: {
        create: jest
          .fn()
          .mockResolvedValue({ id: "re_1", status: "succeeded", amount: 1000 }),
      },
      charges: {
        retrieve: jest
          .fn()
          .mockResolvedValue({ id: "ch_1", receipt_url: "http://receipt" }),
      },
    ...
    (OrgBilling.findOneAndUpdate as jest.Mock).mockReturnValue(
      mockChain(mockDoc({})),
    );
  ...
  describe("Connect & Onboarding", () => {
    ...
    it("getAccountStatus: throws if org not found", async () => {
      await expect(StripeService.getAccountStatus("bad-id")).rejects.toThrow(
    ...
    it("createOnboardingLink: creates session", async () => {
      ...
      (OrgBilling.findOne as jest.Mock).mockReturnValue(
        mockChain({ connectAccountId: "acct_1" }),
      );
    ...
    it("createOnboardingLink: throws if no connect account", async () => {
      ...
      (OrgBilling.findOne as jest.Mock).mockReturnValue(mockChain(null));
      await expect(StripeService.createOnboardingLink("org-1")).rejects.toThrow(
  ...
  describe("Payment Intents (Appointment & Invoice)", () => {
    it("createPaymentIntentForAppointment: creates intent", async () => {
      ...
      (ServiceModel.findById as jest.Mock).mockReturnValue(
        mockChain({ cost: 100 }),
      );
      (OrganizationModel.findById as jest.Mock).mockReturnValue(
        mockChain({ stripeAccountId: "acct_1" }),
      );

      const res =
        await StripeService.createPaymentIntentForAppointment("appt-1");
    ...
    it("createCheckoutSessionForInvoice: creates session and updates invoice", async () => {
      (InvoiceModel.findById as jest.Mock).mockReturnValue(
        mockChain({
          ...
          status: "PENDING",
          organisationId: "org-1",
          items: [{ name: "Item 1", unitPrice: 100, quantity: 1 }],
        }),
      );
      (OrganizationModel.findById as jest.Mock).mockReturnValue(
        mockChain({ stripeAccountId: "acct_1" }),
  ...
  describe("Webhooks", () => {
    ...
    it("handleWebhookEvent: invoice.paid", async () => {
      const event = {
        type: "invoice.paid",
        data: { object: { lines: { data: [{ subscription: "sub_1" }] } } },
      } as any;
      await StripeService.handleWebhookEvent(event);
      expect(OrgBilling.updateOne).toHaveBeenCalledWith(
    ...
    it("handleWebhookEvent: payment_intent.succeeded (Appointment)", async () => {
      ...
      (ServiceModel.findById as jest.Mock).mockReturnValue(
        mockChain({ cost: 50, name: "Svc", description: "desc" }),
      );
