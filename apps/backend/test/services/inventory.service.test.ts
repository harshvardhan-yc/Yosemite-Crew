...
const mockMongooseChain = (resolvedValue: any) => {
  ...
  chain.then = (resolve: any, reject: any) => chain.exec().then(resolve, reject);
  return chain;
};
...
const mockDoc = (data: any) => ({
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  save: (jest.fn() as any).mockResolvedValue({ ...data, _id: data._id || new Types.ObjectId() }),
...
describe('Inventory Module', () => {
  ...
  describe('InventoryService', () => {
    ...
    describe('createItem', () => {
      it('should throw if required fields are missing', async () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await expect(InventoryService.createItem({} as any)).rejects.toThrow('organisationId is required');
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await expect(InventoryService.createItem({ organisationId: orgId } as any)).rejects.toThrow('name is required');
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        await expect(InventoryService.createItem({ organisationId: orgId, name: 'Item' } as any)).rejects.toThrow('category is required');
      });
      ...
      it('should create an item without batches', async () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (InventoryItemModel.create as any).mockResolvedValue(mockDoc({ ...input, _id: itemId }));
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (InventoryBatchModel.find as any).mockReturnValue(mockMongooseChain([]));

        const result = await InventoryService.createItem(input);

        expect(InventoryItemModel.create).toHaveBeenCalledWith(expect.objectContaining({ name: 'Test Item' }));
      ...
      it('should create an item WITH batches and recompute stock', async () => {
        ...
        const itemDoc = mockDoc({ ...input, _id: itemId, onHand: 0, allocated: 0 });
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (InventoryItemModel.create as any).mockResolvedValue(itemDoc);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (InventoryBatchModel.insertMany as any).mockResolvedValue(input.batches);
    ...
    describe('updateItem', () => {
      it('should throw if item not found', async () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (InventoryItemModel.findById as any).mockReturnValue(mockMongooseChain(null));
        await expect(InventoryService.updateItem(itemId, {})).rejects.toThrow('Inventory item not found');
      });

      it('should update specific fields', async () => {
        const itemDoc = mockDoc({ _id: itemId, name: 'Old', attributes: {} });
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (InventoryItemModel.findById as any).mockReturnValue(mockMongooseChain(itemDoc));
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (InventoryBatchModel.find as any).mockReturnValue(mockMongooseChain([]));
    ...
    describe('Status Updates (hide, archive, active)', () => {
        ...
        it('should throw if item to hide not found', async () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryItemModel.findById as any).mockReturnValue(mockMongooseChain(null));
            await expect(InventoryService.hideItem(itemId)).rejects.toThrow('not found');
        });
        ...
        it('should throw if item to archive not found', async () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryItemModel.findById as any).mockReturnValue(mockMongooseChain(null));
            await expect(InventoryService.archiveItem(itemId)).rejects.toThrow('not found');
        });
        ...
        it('should throw if item to activate not found', async () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryItemModel.findById as any).mockReturnValue(mockMongooseChain(null));
            await expect(InventoryService.activeItem(itemId)).rejects.toThrow('not found');
        });

        it('should throw on invalid ObjectId', async () => {
             await expect(InventoryService.activeItem('bad-id')).rejects.toThrow('Invalid itemId');
        });
    });
    ...
    describe('getItemWithBatches', () => {
        it('should return item and batches', async () => {
            ...
            (InventoryItemModel.findById as any).mockReturnValue(mockMongooseChain(itemDoc));
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryBatchModel.find as any).mockReturnValue(mockMongooseChain(['batch1']));
        ...
        it('should throw if item missing', async () => {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryBatchModel.find as any).mockReturnValue(mockMongooseChain([]));
            await expect(InventoryService.getItemWithBatches(itemId, orgId)).rejects.toThrow('not found');
        });
    });
    ...
    describe('Batch Operations', () => {
        it('addBatch: should create batch and update item', async () => {
            const itemDoc = mockDoc({ _id: itemId, organisationId: orgId, save: jest.fn() });
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryItemModel.findById as any).mockResolvedValue(itemDoc);
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryBatchModel.create as any).mockResolvedValue({ quantity: 10, allocated: 0 } as any);
            // recompute helper mock
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryBatchModel.find as any).mockReturnValue(mockMongooseChain([{ quantity: 10, allocated: 0 }]));
        ...
        it('addBatch: should throw if item not found', async () => {
            ...
            await expect(InventoryService.addBatch(itemId, { quantity: 1 })).rejects.toThrow('not found');
        });

        it('updateBatch: should update batch and recompute item', async () => {
            const batchDoc = mockDoc({ _id: batchId, itemId: new Types.ObjectId(itemId), quantity: 5, save: jest.fn() });
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryBatchModel.findById as any).mockReturnValue(mockMongooseChain(batchDoc));
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryBatchModel.find as any).mockReturnValue(mockMongooseChain([{ quantity: 10 }])); // Mock sum
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryItemModel.findByIdAndUpdate as any).mockReturnValue(mockMongooseChain({}));
        ...
        it('updateBatch: should throw if batch not found', async () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryBatchModel.findById as any).mockReturnValue(mockMongooseChain(null));
            await expect(InventoryService.updateBatch(batchId, {})).rejects.toThrow('Batch not found');
        });

        it('deleteBatch: should delete batch and recompute', async () => {
             const batchDoc = mockDoc({ _id: batchId, itemId: new Types.ObjectId(itemId) });
             // eslint-disable-next-line @typescript-eslint/no-explicit-any
             (InventoryBatchModel.findById as any).mockReturnValue(mockMongooseChain(batchDoc));
             // eslint-disable-next-line @typescript-eslint/no-explicit-any
             (InventoryBatchModel.find as any).mockReturnValue(mockMongooseChain([]));
    ...
    describe('consumeStock', () => {
        it('should throw if qty <= 0', async () => {
            await expect(InventoryService.consumeStock({ itemId, quantity: 0, reason: 'OTHER' })).rejects.toThrow('quantity must be > 0');
        });

        it('should throw if item not found', async () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryItemModel.findById as any).mockReturnValue(mockMongooseChain(null));
            await expect(InventoryService.consumeStock({ itemId, quantity: 1, reason: 'OTHER' })).rejects.toThrow('not found');
        });

        it('should throw if insufficient stock', async () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryItemModel.findById as any).mockReturnValue(mockMongooseChain({ onHand: 5 }));
            await expect(InventoryService.consumeStock({ itemId, quantity: 10, reason: 'OTHER' })).rejects.toThrow('Insufficient stock');
        });
        ...
        it('should consume stock FIFO', async () => {
            ...
            (InventoryItemModel.findById as any).mockReturnValue(mockMongooseChain(itemDoc));
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryBatchModel.find as any).mockReturnValueOnce(mockMongooseChain([b1, b2])); // For consumption loop

            // For recompute
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryBatchModel.find as any).mockReturnValue(mockMongooseChain([{ quantity: 7 }]));

            await InventoryService.consumeStock({ itemId, quantity: 8, reason: 'OTHER' });
        ...
        it('should throw 500 if consumption fails logic', async () => {
            ...
            await expect(InventoryService.consumeStock({ itemId, quantity: 50, reason: 'OTHER' }))
  ...
  describe('InventoryAdjustmentService', () => {
     describe('adjustStock', () => {
        it('should create batch for positive adjustment', async () => {
             ...
             // recompute
             // eslint-disable-next-line @typescript-eslint/no-explicit-any
             (InventoryBatchModel.find as any).mockResolvedValue([{ quantity: 15, allocated: 0 }] as any);

             // New onHand 15 (delta +5)
             await InventoryAdjustmentService.adjustStock({ itemId, newOnHand: 15, reason: 'Fix' });
        ...
        it('should consume batches for negative adjustment', async () => {
            ...
            (InventoryItemModel.findById as any).mockResolvedValue(itemDoc as any);
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (InventoryBatchModel.find as any).mockReturnValue(mockMongooseChain([batch]));
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (StockMovementModel.create as any).mockResolvedValue({} as any);

            // New onHand 5 (delta -5)
            await InventoryAdjustmentService.adjustStock({ itemId, newOnHand: 5, reason: 'Fix' });
        ...
        it('should throw if insufficient stock for negative adjustment', async () => {
             ...
             await expect(InventoryAdjustmentService.adjustStock({ itemId, newOnHand: 5, reason: 'Fix' }))
        ...
        it('should throw if item not found', async () => {
            ...
            await expect(InventoryAdjustmentService.adjustStock({ itemId, newOnHand: 5, reason: '' }))
  ...
  describe('InventoryAllocationService', () => {
     it('allocateStock: should allocate if stock exists', async () => {
         ...
         await InventoryAllocationService.allocateStock({ itemId, quantity: 5, referenceId: 'ref' });
     ...
     it('allocateStock: should throw if not enough free stock', async () => {
        ...
        await expect(InventoryAllocationService.allocateStock({ itemId, quantity: 5, referenceId: 'ref' }))
     ...
     it('allocateStock: should throw if item not found', async () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (InventoryItemModel.findById as any).mockResolvedValue(null as any);
        await expect(InventoryAllocationService.allocateStock({ itemId, quantity: 1, referenceId: 'ref' }))
            .rejects.toThrow('Item not found');
     });
     ...
     it('releaseAllocatedStock: should decrease allocated', async () => {
         ...
         await InventoryAllocationService.releaseAllocatedStock({ itemId, quantity: 3, referenceId: 'ref' });
         expect(itemDoc.allocated).toBe(2);
     });
     ...
     it('releaseAllocatedStock: should throw if item not found', async () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (InventoryItemModel.findById as any).mockResolvedValue(null as any);
        await expect(InventoryAllocationService.releaseAllocatedStock({ itemId, quantity: 1, referenceId: 'ref' }))
            .rejects.toThrow('Item not found');
     });
  });
  ...
  describe('InventoryVendorService', () => {
     it('createVendor: success', async () => {
         // eslint-disable-next-line @typescript-eslint/no-explicit-any
         (InventoryVendorModel.create as any).mockResolvedValue({ _id: 'v1' } as any);
         const res = await InventoryVendorService.createVendor({ organisationId: orgId, name: 'V' });
         expect(res).toEqual({ _id: 'v1' });
     });
     it('createVendor: missing orgId', async () => {
         // eslint-disable-next-line @typescript-eslint/no-explicit-any
         await expect(InventoryVendorService.createVendor({ name: 'V' } as any)).rejects.toThrow('organisationId required');
     });
     ...
     it('updateVendor: not found', async () => {
         ...
         await expect(InventoryVendorService.updateVendor(validVendorId, {})).rejects.toThrow('Vendor not found');
     });
     ...
     it('listVendors', async () => {
         ...
         expect(InventoryVendorModel.find).toHaveBeenCalledWith({ organisationId: orgId });
     });
     ...
     it('deleteVendor', async () => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (InventoryVendorModel.findByIdAndDelete as any).mockResolvedValue({} as any);
  ...
  describe('InventoryMetaFieldService', () => {
      ...
      it('updateField: success', async () => {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          await InventoryMetaFieldService.updateField(new Types.ObjectId().toString(), { label: 'New' } as any);
      ...
      it('updateField: not found', async () => {
          ...
          await expect(InventoryMetaFieldService.updateField(new Types.ObjectId().toString(), {})).rejects.toThrow('not found');
      });

      it('deleteField', async () => {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (InventoryMetaFieldModel.findByIdAndDelete as any).mockResolvedValue({} as any);
          await InventoryMetaFieldService.deleteField(new Types.ObjectId().toString());
          expect(InventoryMetaFieldModel.findByIdAndDelete).toHaveBeenCalled();
      });

      it('listFields', async () => {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          (InventoryMetaFieldModel.find as any).mockReturnValue(mockMongooseChain([]));
          await InventoryMetaFieldService.listFields('GENERAL');
          expect(InventoryMetaFieldModel.find).toHaveBeenCalledWith({ businessType: 'GENERAL' });
      });
  });
  ...
  describe('Utils', () => {
      it('ensureObjectId throws on invalid', async () => {
          await expect(InventoryService.updateItem('bad-id', {})).rejects.toThrow('Invalid itemId');
      });
  });
