...
const mockDoc = (data: any = {}) => {
  const _id = data._id
    ? data._id instanceof Types.ObjectId
      ? data._id
...
describe("Inventory Services", () => {
  ...
  describe("InventoryService", () => {
    describe("createItem", () => {
      it("should throw if required fields missing", async () => {
        ...
        await expect(
          InventoryService.createItem({ organisationId: "1" } as any),
    ...
    describe("updateItem", () => {
      it("should throw if item not found", async () => {
        ...
        await expect(
          InventoryService.updateItem(VALID_ID_STR, {}),
    ...
    describe("State Changes", () => {
      ...
      it("should throw if item not found during state change", async () => {
        ...
        await expect(InventoryService.hideItem(VALID_ID_STR)).rejects.toThrow(
    ...
    describe("getItemWithBatches", () => {
      it("should return item and batches", async () => {
        (InventoryItemModel.findById as jest.Mock).mockReturnValue(
          mockChain(mockDoc({ name: "I" })),
        );
        (InventoryBatchModel.find as jest.Mock).mockReturnValue(mockChain([]));
        const res = await InventoryService.getItemWithBatches(
          VALID_ID_STR,
          VALID_ORG_ID,
        );
      ...
      it("should throw if not found", async () => {
        ...
        await expect(
          InventoryService.getItemWithBatches(VALID_ID_STR, VALID_ORG_ID),
    ...
    describe("Batch Operations", () => {
      it("should add batch and recompute item stock", async () => {
        ...
        (InventoryBatchModel.find as jest.Mock).mockReturnValue(
          mockChain([mockDoc({ quantity: 10, allocated: 5 })]),
        );
      ...
      it("should throw if adding batch to missing item", async () => {
        ...
        await expect(
          InventoryService.addBatch(VALID_ID_STR, { quantity: 1 }),
        ).rejects.toThrow("not found");
      });

      it("should update batch and recompute", async () => {
        const batch = mockDoc({ itemId: VALID_ID_OBJ });
        (InventoryBatchModel.findById as jest.Mock).mockReturnValue(
          mockChain(batch),
        );
        (InventoryBatchModel.find as jest.Mock).mockReturnValue(
          mockChain([{ quantity: 20 }]),
        );
        ...
        expect(InventoryItemModel.findByIdAndUpdate).toHaveBeenCalledWith(
          VALID_ID_OBJ,
          { onHand: 20, allocated: 0 },
        );
      });
      ...
      it("should throw if updating missing batch", async () => {
        ...
        await expect(
          InventoryService.updateBatch(VALID_ID_STR, {}),
  ...
  describe("InventoryAdjustmentService", () => {
    it("should handle positive adjustment", async () => {
      ...
      expect(StockMovementModel.create).toHaveBeenCalledWith(
        expect.objectContaining({ change: 5 }),
      );
  ...
  describe("InventoryMetaFieldService", () => {
    ...
    it("listFields", async () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (InventoryMetaFieldModel.find as any).mockReturnValue(mockChain([]));
  ...
  describe("InventoryMetaFieldService", () => {
    ...
    it("should throw if field missing", async () => {
      ...
      await expect(
        InventoryMetaFieldService.updateField(VALID_ID_STR, {}),
      ).rejects.toThrow("Meta field not found");
    });
    ...
    it("should list fields", async () => {
      (InventoryMetaFieldModel.find as jest.Mock).mockReturnValue(
        mockChain([]),
      );
      await InventoryMetaFieldService.listFields("GENERAL");
      expect(InventoryMetaFieldModel.find).toHaveBeenCalledWith({
