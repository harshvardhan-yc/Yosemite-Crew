...
describe('Task Model', () => {
  ...
  describe('Schema Validation', () => {
    ...
    it('should validate Enums', () => {
      const task = new TaskModel({
        createdBy: 'u1', assignedTo: 'u2', dueAt: new Date(),
  ...
  describe('Medication Validation', () => {
    it('should require doses array to be non-empty if medication is present', () => {
      const task = new TaskModel({
        createdBy: 'u1', assignedTo: 'u2', audience: 'PARENT_TASK', source: 'CUSTOM', dueAt: new Date(),
        medication: {
          name: 'Pill',
      ...
      expect(err?.errors['medication.doses'].message).toContain('At least one dose is required');
    });
