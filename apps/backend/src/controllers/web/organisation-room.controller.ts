...
export const OrganisationRoomController = {
    create: async (req: Request, res: Response) => {
        try {
            ...
            const { response, created } = await OrganisationRoomService.create(payload)
    ...
    getAllByOrganizationId: async (req: Request, res: Response) => {
        try {
            ...
            const resources = await OrganisationRoomService.getAllByOrganizationId(organizationId)
            ...
            res.status(500).json({ message: 'Unable to retrieve organisation rooms.' })
        }
    },
}
