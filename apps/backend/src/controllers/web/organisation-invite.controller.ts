...
type CreateInviteBody = Omit<CreateInvitePayload, 'organisationId' | 'invitedByUserId'> & {
    departmentId?: string
    inviteeEmail?: string
    inviteeName?: string
    role?: string
...
export const OrganisationInviteController = {
    createInvite: async (req: Request, res: Response) => {
        try {
            ...
            res.status(500).json({ message: 'Unable to create organisation invite.' })
        }
    },
    ...
    listOrganisationInvites: async (req: Request, res: Response) => {
        try {
            ...
            const invites = await OrganisationInviteService.listOrganisationInvites(organisationId)
    ...
    acceptInvite: async (req: Request, res: Response) => {
        try {
            ...
            if (!userId || !userEmail) {
                res.status(401).json({ message: 'Authenticated user information is required.' })
                return
            }
            ...
            res.status(500).json({ message: 'Unable to accept organisation invite.' })
        }
    },
}
