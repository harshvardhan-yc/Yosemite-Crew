...
type CreateAvailabilityRequest = Request<Record<string, never>, unknown, unknown>
type UpdateAvailabilityRequest = Request<{ userId: string }, unknown, unknown>
type GetAvailabilityRequest = Request<{ userId: string }>

function ensureBodyObject(body: unknown): asserts body is Record<string, unknown> {
...
export const BaseAvailabilityController = {
    create: async (req: CreateAvailabilityRequest, res: Response) => {
        try {
            ...
            const availability = await BaseAvailabilityService.create(requestBody as CreateBaseAvailabilityPayload)
    ...
    getByUserId: async (req: GetAvailabilityRequest, res: Response) => {
        try {
            const availability = await BaseAvailabilityService.getByUserId(req.params.userId)
            ...
            res.status(500).json({ message: 'Unable to retrieve base availability.' })
        }
    },
}
