...
export const SpecialityController = {
    ...
    createMany: async (req: Request, res: Response) => {
        try {
            ...
            if (!Array.isArray(payloads) || !payloads.every(isFHIRSpecialityPayload)) {
                res.status(400).json({
                    message: 'Invalid payload list. Expected array of FHIR Organization resources.',
                })
                return
            }
    ...
    getAllByOrganizationId: async (req: Request, res: Response) => {
        try {
            ...
            const resources = await SpecialityService.getAllByOrganizationId(organizationId)
