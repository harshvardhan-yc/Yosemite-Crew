...
export type OrganisationInviteDocument =
  HydratedDocument<OrganisationInviteMongo>;
...
export interface OrganisationInviteModel
  ...
  findValidInviteByToken(
    token: string,
    referenceDate?: Date,
  ): Promise<OrganisationInviteDocument | null>;
...
OrganisationInviteSchema.statics.generateToken = function generateToken(
  byteLength: number = INVITE_TOKEN_BYTES,
) {
...
OrganisationInviteSchema.statics.findValidInviteByToken =
  function findValidInviteByToken(
    token: string,
...
const OrganisationInviteModel = model<
  OrganisationInviteMongo,
  OrganisationInviteModel
>("OrganisationInvite", OrganisationInviteSchema);
