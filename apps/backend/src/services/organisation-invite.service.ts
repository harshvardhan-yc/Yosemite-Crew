...
export type CreateInvitePayload = Omit<CreateOrganisationInviteInput, 'organisationId'> & {
...
const requireString = (value: unknown, fieldName: string): string => {
    ...
    if (typeof value !== 'string') {
        throw new OrganisationInviteServiceError(`${fieldName} must be a string.`, 400)
    }

    const trimmed = value.trim()

    if (!trimmed) {
        throw new OrganisationInviteServiceError(`${fieldName} cannot be empty.`, 400)
    }

    if (trimmed.includes('$')) {
        throw new OrganisationInviteServiceError(`Invalid character in ${fieldName}.`, 400)
    }
...
const normalizeIdentifier = (value: unknown, fieldName: string): string => {
    ...
    if (!Types.ObjectId.isValid(identifier) && !IDENTIFIER_PATTERN.test(identifier)) {
        throw new OrganisationInviteServiceError(`Invalid ${fieldName.toLowerCase()} format.`, 400)
    }
...
const normalizeEmail = (value: unknown): string => {
    ...
    if (!validator.isEmail(email)) {
        throw new OrganisationInviteServiceError('Invalid invitee email address.', 400)
    }
...
const validateEmploymentType = (value: unknown) => {
    ...
    if (value === 'FULL_TIME' || value === 'PART_TIME' || value === 'CONTRACTOR') {
        return value
    }

    throw new OrganisationInviteServiceError('Invalid employment type supplied.', 400)
}
...
const buildIdentifierLookup = (identifier: string) => {
    ...
    if (!predicates.length) {
        throw new OrganisationInviteServiceError('Unable to build identifier lookup.', 400)
    }

    return predicates.length === 1 ? predicates[0] : { $or: predicates }
}

const buildInviteResponse = (document: OrganisationInviteDocument): OrganisationInviteResponse => {
...
const findOrganisationOrThrow = async (organisationId: string): Promise<OrganizationMongo> => {
    const query = buildIdentifierLookup(organisationId)
    const organisation = await OrganizationModel.findOne(query).setOptions({ sanitizeFilter: true })
...
const ensureDepartmentBelongsToOrganisation = async (
    ...
    const department = await SpecialityModel.findOne({ ...query, organisationId }).setOptions({
        sanitizeFilter: true,
    })

    if (!department) {
        throw new OrganisationInviteServiceError('Department not found for the organisation.', 404)
    }

    return department
}

const ensureUserOrganizationMembership = async (organisationId: string, role: string, userId: string) => {
    const practitionerReference = userId.startsWith('Practitioner/') ? userId : `Practitioner/${userId}`
    ...
    try {
        ...
        if (duplicateKey) {
            logger.info('User already associated with organisation role; skipping duplicate creation.', {
                organisationId,
                practitionerReference,
                role,
            })
...
const addUserToDepartment = async (department: SpecialityDocument, userId: string) => {
...
const buildAcceptInviteUrl = (token: string): string => {
    ...
    if (!trimmedBase) {
        throw new OrganisationInviteServiceError('Invite acceptance URL is not configured.', 500)
    }
...
const sendInviteEmail = async (invite: OrganisationInviteDocument, organisation: OrganizationMongo) => {
...
export const OrganisationInviteService = {
    async createInvite(payload: CreateInvitePayload): Promise<OrganisationInviteResponse> {
        const organisationId = normalizeIdentifier(payload.organisationId, 'Organisation identifier')
        const departmentId = normalizeIdentifier(payload.departmentId, 'Department identifier')
        const invitedByUserId = requireString(payload.invitedByUserId, 'Inviter identifier')
        const inviteeEmail = normalizeEmail(payload.inviteeEmail)
        const inviteeName = payload.inviteeName ? requireString(payload.inviteeName, 'Invitee name') : undefined
        ...
        try {
            ...
            throw new OrganisationInviteServiceError('Unable to send organisation invite email.', 502)
        }

        return buildInviteResponse(invite)
    },

    async listOrganisationInvites(organisationIdInput: string): Promise<OrganisationInviteResponse[]> {
        const organisationId = normalizeIdentifier(organisationIdInput, 'Organisation identifier')
    ...
    async acceptInvite({ token, userId, userEmail }: AcceptInvitePayload): Promise<OrganisationInviteResponse> {
        ...
        const invite = await OrganisationInviteModel.findOne({ token: safeToken }).setOptions({
            sanitizeFilter: true,
        })
        ...
        if (invite.status === 'ACCEPTED') {
            throw new OrganisationInviteServiceError('Invitation already accepted.', 409)
        }

        if (invite.status === 'CANCELLED') {
            throw new OrganisationInviteServiceError('Invitation has been cancelled.', 410)
        }
        ...
        if (invite.inviteeEmail !== safeEmail) {
            throw new OrganisationInviteServiceError('Invite email does not match authenticated user.', 403)
        }

        await findOrganisationOrThrow(invite.organisationId)
        const department = await ensureDepartmentBelongsToOrganisation(invite.departmentId, invite.organisationId)
        ...
        try {
            await ensureUserOrganizationMembership(invite.organisationId, invite.role, safeUserId)
        } catch (error) {
            if (error instanceof OrganisationInviteServiceError) {
                throw error
            }
            logger.error('Failed to ensure user-organisation membership during invite acceptance.', error)
            throw new OrganisationInviteServiceError('Unable to associate user with organisation.', 500)
        }
