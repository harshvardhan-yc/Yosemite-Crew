...
const requireOrganizationId = (value: unknown): string => {
    ...
    if (!isValidObjectId(identifier) && !/^[A-Za-z0-9\-.]{1,64}$/.test(identifier)) {
        throw new SpecialityServiceError('Invalid organisation identifier format.', 400)
    }
...
const sanitizeSpecialityAttributes = (dto: SpecialityDTOAttributes): SpecialityMongo => {
    ...
    return {
        ...
        organisationId,
        departmentMasterId: optionalSafeString(dto.departmentMasterId, 'Department master identifier'),
        name,

        headUserId: optionalSafeString(dto.headUserId, 'Head user identifier'),
        headName: optionalSafeString(dto.headName, 'Head name'),
        headProfilePicUrl: optionalSafeString(dto.headProfilePicUrl, 'Head profile picture URL'),
...
const buildDomainSpeciality = (document: SpecialityDocument) => {
    const { _id, ...rest } = document.toObject({ virtuals: false }) as SpecialityMongo & {
        _id: Types.ObjectId
    }
...
const buildFHIRResponse = (document: SpecialityDocument): SpecialityResponseDTO => {
...
const createPersistableFromFHIR = (payload: SpecialityFHIRPayload) => {
    if (payload?.resourceType !== 'Organization') {
        throw new SpecialityServiceError('Invalid payload. Expected FHIR Organization resource.', 400)
    }
...
const resolveIdQuery = (id: unknown): { _id?: string; fhirId?: string } => {
    ...
    throw new SpecialityServiceError('Invalid speciality identifier format.', 400)
}
...
export const SpecialityService = {
    ...
    async getAllByOrganizationId(organisationId: string) {
        ...
        const documents = await SpecialityModel.find({ organisationId: orgId }).exec()
