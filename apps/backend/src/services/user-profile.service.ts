...
import type { UserProfile as UserProfileType, UserAvailability } from '@yosemite-crew/types'
import {
...
const optionalEnum = <T extends string>(value: unknown, allowed: readonly T[], field: string): T | undefined => {
    if (value == null) {
        return undefined
    }
    ...
    if (!matches) {
        throw new UserProfileServiceError(`${field} must be one of: ${allowed.join(', ')}.`, 400)
    }
...
const optionalNumber = (value: unknown, field: string): number | undefined => {
    ...
    if (typeof value === 'number') {
        if (Number.isNaN(value)) {
            throw new UserProfileServiceError(`${field} must be a valid number.`, 400)
        }
...
const sanitizeDocuments = (value: unknown): UserProfileDocumentMongo[] | undefined => {
    if (value == null) {
        return undefined
    }

    if (!Array.isArray(value)) {
        throw new UserProfileServiceError('Professional documents must be an array.', 400)
    }
    ...
    return value.map((item, index) => {
        ...
        if (!type) {
            throw new UserProfileServiceError(`Professional document[${index}].type is required.`, 400)
        }

        const fileUrl = requireString(record.fileUrl, `Professional document[${index}].fileUrl`)
        const uploadedAt = optionalDate(record.uploadedAt, `Professional document[${index}].uploadedAt`)

        if (!uploadedAt) {
            throw new UserProfileServiceError(`Professional document[${index}].uploadedAt is required.`, 400)
        }
        ...
        return pruneUndefined({
            ...
            fileUrl,
            uploadedAt,
            verified: optionalBoolean(record.verified, `Professional document[${index}].verified`),
        })
    })
}

const sanitizePersonalDetails = (value: unknown): UserProfilePersonalDetailsMongo | undefined => {
    if (value == null) {
        return undefined
    }
    ...
    return pruneUndefined({
        gender: optionalEnum(record.gender, ['MALE', 'FEMALE', 'OTHER'] as const, 'Gender'),
        ...
        profilePictureUrl: optionalString(record.profilePictureUrl, 'Profile picture URL'),
    })
}

const sanitizeProfessionalDetails = (value: unknown): UserProfileProfessionalDetailsMongo | undefined => {
    if (value == null) {
        return undefined
    }
    ...
    return pruneUndefined({
        medicalLicenseNumber: optionalString(record.medicalLicenseNumber, 'Medical license number'),
        yearsOfExperience: optionalNumber(record.yearsOfExperience, 'Years of experience'),
...
const isPersonalDetailsComplete = (details: UserProfileMongo['personalDetails']): boolean => {
    if (!details) {
        return false
    }
...
const isProfessionalDetailsComplete = (details: UserProfileMongo['professionalDetails']): boolean => {
    if (!details) {
        return false
    }
...
const determineProfileStatus = (
    ...
    const professionalComplete = isProfessionalDetailsComplete(profile.professionalDetails)
    const availabilityComplete = hasValidAvailability(availability)

    return personalComplete && professionalComplete && availabilityComplete ? 'COMPLETED' : 'DRAFT'
}
...
const buildDomainProfile = (
    ...
    const id =
        typeof idSource === 'string'
            ? idSource
            : typeof idSource === 'object' && idSource !== null && 'toString' in idSource
    ...
    const personalDetails = raw.personalDetails
        ? pruneUndefined({
              ...raw.personalDetails,
              address: raw.personalDetails.address ? pruneUndefined(raw.personalDetails.address) : undefined,
          })
        : undefined
    ...
    const professionalDetails = raw.professionalDetails
        ? pruneUndefined({
              ...
              documents: raw.professionalDetails.documents
                  ? raw.professionalDetails.documents.map((documentItem) => pruneUndefined(documentItem))
                  : undefined,
          })
        : undefined
...
const sanitizeCreatePayload = (
    ...
    const professionalDetails = sanitizeProfessionalDetails(payload.professionalDetails)
...
const sanitizeUpdatePayload = (
    ...
    if ('personalDetails' in payload) {
        sanitized.personalDetails = sanitizePersonalDetails(payload.personalDetails)
        hasProfileUpdate = true
    }

    if ('professionalDetails' in payload) {
        sanitized.professionalDetails = sanitizeProfessionalDetails(payload.professionalDetails)
        hasProfileUpdate = true
    }
    ...
    return {
        ...
        baseAvailability: hasAvailabilityUpdate ? payload.baseAvailability : undefined,
    }
}
