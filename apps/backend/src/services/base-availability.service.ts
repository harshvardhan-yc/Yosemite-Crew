...
const forbidQueryOperators = (value: string, field: string) => {
    if (value.includes('$')) {
        throw new BaseAvailabilityServiceError(`Invalid character in ${field}.`, 400)
    }
}
...
const sanitizeSlot = (value: unknown, index: number): AvailabilitySlotMongo => {
    ...
    if (!isValidTime(startTime) || !isValidTime(endTime)) {
        throw new BaseAvailabilityServiceError(`Slot[${index}] times must be in HH:MM format.`, 400)
    }

    if (startTime >= endTime) {
        throw new BaseAvailabilityServiceError(`Slot[${index}].startTime must be before endTime.`, 400)
    }
    ...
    const isAvailable =
        'isAvailable' in record
            ? typeof record.isAvailable === 'boolean'
                ? record.isAvailable
                : (() => {
                      throw new BaseAvailabilityServiceError(`Slot[${index}].isAvailable must be a boolean.`, 400)
                  })()
            : true
...
const sanitizeAvailabilityEntry = (value: unknown, index: number): BaseAvailabilityMongo => {
    const record = assertPlainObject(value, `Availability[${index}]`)
    const dayOfWeekValue = requireString(record.dayOfWeek, `Availability[${index}].dayOfWeek`)

    const allowedDays: DayOfWeek[] = ['MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY', 'SATURDAY', 'SUNDAY']
    ...
    if (!Array.isArray(rawSlots)) {
        throw new BaseAvailabilityServiceError(`Availability[${index}].slots must be an array.`, 400)
    }

    const slots = rawSlots.map((slot, slotIndex) => sanitizeSlot(slot, slotIndex))

    if (!slots.length) {
        throw new BaseAvailabilityServiceError(`Availability[${index}] must contain at least one slot.`, 400)
    }

    const recordUserId = 'userId' in record ? requireString(record.userId, `Availability[${index}].userId`) : undefined
...
const buildDomainAvailability = (document: BaseAvailabilityDocument): UserAvailability => {
    const raw = document.toObject({ virtuals: false }) as BaseAvailabilityMongo & { _id: unknown }
...
const sanitizeCreatePayload = (payload: CreateBaseAvailabilityPayload): {
    userId: string
    ...
    if (!payload.availability || !Array.isArray(payload.availability)) {
        throw new BaseAvailabilityServiceError('Availability must be an array.', 400)
    }
    ...
    if (!entries.length) {
        throw new BaseAvailabilityServiceError('Availability cannot be empty.', 400)
    }

    return { userId, availability: entries }
}

const sanitizeUpdatePayload = (userId: unknown, payload: UpdateBaseAvailabilityPayload): {
    userId: string
    availability: BaseAvailabilityMongo[]
} => {
...
const sortByDayOrder = (availabilities: UserAvailability[]): UserAvailability[] => {
    ...
    return [...availabilities].sort((a, b) => (order[a.dayOfWeek] ?? 0) - (order[b.dayOfWeek] ?? 0))
}

export const BaseAvailabilityService = {
    async create(payload: CreateBaseAvailabilityPayload): Promise<UserAvailability[]> {
        const { userId, availability } = sanitizeCreatePayload(payload)

        const existing = await BaseAvailabilityModel.findOne({ userId }, null, { sanitizeFilter: true })

        if (existing) {
            throw new BaseAvailabilityServiceError('Base availability already exists for this user.', 409)
        }
    ...
    async update(userId: unknown, payload: UpdateBaseAvailabilityPayload): Promise<UserAvailability[]> {
        const { userId: identifier, availability } = sanitizeUpdatePayload(userId, payload)
    ...
    async getByUserId(userId: unknown): Promise<UserAvailability[]> {
        ...
        const documents = await BaseAvailabilityModel.find({ userId: identifier }).sort({ dayOfWeek: 1 })
