...
const buildExtensions = (schedule: TaskScheduleLike): Extension[] =>
  [
    buildExtension(
      TASK_SCHEDULE_TEMPLATE_INSTANCE_EXTENSION_URL,
      schedule.templateInstanceId,
    ),
    buildExtension(TASK_SCHEDULE_TEMPLATE_EXTENSION_URL, schedule.templateId),
    buildExtension(
      TASK_SCHEDULE_TEMPLATE_VERSION_EXTENSION_URL,
      schedule.templateVersion,
    ),
    buildExtension(
      TASK_SCHEDULE_TEMPLATE_KIND_EXTENSION_URL,
      schedule.templateKind,
    ),
    buildExtension(
      TASK_SCHEDULE_ORGANISATION_EXTENSION_URL,
      schedule.organisationId,
    ),
    buildExtension(TASK_SCHEDULE_CREATED_BY_EXTENSION_URL, schedule.createdBy),
    buildExtension(
      TASK_SCHEDULE_ACTIVATED_BY_EXTENSION_URL,
      schedule.activatedBy,
    ),
    buildExtension(
      TASK_SCHEDULE_ACTIVATED_AT_EXTENSION_URL,
      toIso(schedule.activatedAt),
    ),
    buildExtension(
      TASK_SCHEDULE_COMPLETED_AT_EXTENSION_URL,
      toIso(schedule.completedAt),
    ),
    buildExtension(
      TASK_SCHEDULE_SCHEDULE_INPUT_EXTENSION_URL,
      schedule.scheduleInput,
    ),
    buildExtension(
      TASK_SCHEDULE_MATERIALIZED_SEEDS_EXTENSION_URL,
      schedule.materializedSeeds,
    ),
    buildExtension(
      TASK_SCHEDULE_GENERATED_TASK_IDS_EXTENSION_URL,
  ...
  output: buildOutputs(
    generatedTaskIds.length
      ...
      : Array.isArray(schedule.generatedTaskIds)
        ? schedule.generatedTaskIds.filter(
...
const getBooleanParameter = (
  ...
  parameters?.parameter?.find((parameter) => parameter.name === name)
    ?.valueBoolean;

const getDateParameter = (parameters: Parameters | undefined, name: string) => {
  const value = parameters?.parameter?.find(
