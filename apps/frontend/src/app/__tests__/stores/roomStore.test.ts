...
const mockUnitGroup: RoomUnitGroup = {
  id: 'group-1',
  organisationId: 'org-A',
  roomId: 'room-1',
  name: 'Canine ward',
  unitCount: 2,
  size: 'Medium',
  speciesConstraints: ['CANINE'],
  isActive: true,
};

const mockUnit: RoomUnit = {
  id: 'unit-1',
  organisationId: 'org-A',
  roomId: 'room-1',
  unitGroupId: 'group-1',
  code: 'CW-01',
  displayName: 'Canine ward 1',
  isActive: true,
};
...
describe('Organisation Room Store', () => {
  ...
  describe('Upsert Operations', () => {
    ...
    it('handles upsert gracefully when room ID is missing', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('upsertRoom: missing id:', mockRoomNoId);
    ...
    it('handles upsert gracefully when organisation ID is missing', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('upsertRoom: missing organisationId:', mockRoomNoOrg);
  ...
  describe('Room Unit Inventory', () => {
    it('sets and retrieves room unit groups and units by room', () => {
      const store = useOrganisationRoomStore.getState();

      store.setRoomUnitGroupsForOrg('org-A', [mockUnitGroup]);
      store.setRoomUnitsForOrg('org-A', [mockUnit]);

      expect(store.getRoomUnitGroupsByRoomId('room-1')).toEqual([mockUnitGroup]);
      expect(store.getRoomUnitsByRoomId('room-1')).toEqual([mockUnit]);
      expect(store.getRoomUnitsByGroupId('group-1')).toEqual([mockUnit]);
    });

    it('upserts and removes room unit groups and units', () => {
      const store = useOrganisationRoomStore.getState();

      store.upsertRoomUnitGroup(mockUnitGroup);
      store.upsertRoomUnit(mockUnit);

      expect(useOrganisationRoomStore.getState().roomUnitGroupIdsByRoomId['room-1']).toEqual([
        'group-1',
      ]);
      expect(useOrganisationRoomStore.getState().roomUnitIdsByRoomId['room-1']).toEqual(['unit-1']);

      store.removeRoomUnit('unit-1');
      store.removeRoomUnitGroup('group-1');

      expect(useOrganisationRoomStore.getState().roomUnitsById['unit-1']).toBeUndefined();
    ...
    it('clears room units and groups with the room', () => {
      ...
      store.removeRoom('room-1');

      const state = useOrganisationRoomStore.getState();
      expect(state.roomUnitGroupsById).toEqual({});
      expect(state.roomUnitsById).toEqual({});
      expect(state.roomUnitGroupIdsByRoomId['room-1']).toBeUndefined();
      expect(state.roomUnitIdsByRoomId['room-1']).toBeUndefined();
    });
  });
});
