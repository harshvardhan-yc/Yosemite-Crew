...
describe('Service Store', () => {
  ...
  describe('Complex Set & Removal', () => {
    ...
    it('deletes service by id and cleans org/speciality indexes', () => {
      const store = useServiceStore.getState();
      store.setServices([mockService1, mockService2, mockService3]);
      store.deleteServiceById('srv-1');
      const state = useServiceStore.getState();
      expect(state.servicesById['srv-1']).toBeUndefined();
      expect(state.serviceIdsByOrgId['org-A']).toEqual(['srv-2']);
      expect(state.serviceIdsBySpecialityId['spec-X']).toEqual(['srv-3']);
    });

    it('warns when deleting unknown service id', () => {
      const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
      const snapshot = JSON.stringify(useServiceStore.getState());
      useServiceStore.getState().deleteServiceById('missing');
      expect(JSON.stringify(useServiceStore.getState())).toEqual(snapshot);
      expect(warnSpy).toHaveBeenCalled();
      warnSpy.mockRestore();
    });

    it('deletes all services by speciality id', () => {
      const store = useServiceStore.getState();
      store.setServices([mockService1, mockService2, mockService3]);
      store.deleteServicesBySpecialityId('spec-X');
      const state = useServiceStore.getState();
      expect(state.servicesById['srv-1']).toBeUndefined();
      expect(state.servicesById['srv-3']).toBeUndefined();
      expect(state.servicesById['srv-2']).toBeDefined();
      expect(state.serviceIdsBySpecialityId['spec-X']).toBeUndefined();
    });

    it('handles deleting speciality with no services', () => {
      const store = useServiceStore.getState();
      store.setServices([mockService1]);
      store.deleteServicesBySpecialityId('missing-spec');
