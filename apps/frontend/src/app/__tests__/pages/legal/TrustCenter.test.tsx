...
describe("TrustCenter Component", () => {
  ...
  const getButtonsByText = (text: string) => {
    return screen.getAllByText((content, element) => {
      return (
        element?.tagName.toLowerCase() !== "script" && content.includes(text)
      );
    });
  };
  ...
  it("renders the hero section correctly", () => {
    render(<TrustCenter />);
    expect(
      screen.getByText(/Security, Privacy, and Compliance/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/At Yosemite Crew, protecting your veterinary/i),
    ).toBeInTheDocument();
  ...
  it("handles locked resource click to open modal", () => {
    ...
    expect(
      screen.getByText(/You are requesting access to:/i),
    ).toBeInTheDocument();
  });
  ...
  it("renders keyboard interactions for accessibility", () => {
    ...
    // and we changed the implementation to use <button> or div with role="button",
    // let's grab the closest role="button"
    const copyButton = copyTextNodes[0].closest('[role="button"]');
    ...
    if (copyButton) {
      ...
      throw new Error(
        "Could not find clickable copy button for accessibility test",
      );
    }
  });
});
