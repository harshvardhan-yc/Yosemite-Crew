...
describe("companionDocumentService", () => {
  ...
  describe("createCompanionDocument", () => {
    ...
    it("throws error when companionId is missing", async () => {
      ...
      await expect(createCompanionDocument(document, "")).rejects.toThrow(
        "Companion ID missing"
      );
    });
    ...
    it("throws error when API call fails", async () => {
      ...
      await expect(
        createCompanionDocument(document, "comp-123")
  ...
  describe("loadCompanionDocument", () => {
    it("fetches documents from the correct endpoint", async () => {
      ...
      expect(getDataMock).toHaveBeenCalledWith("/v1/document/pms/comp-123");
      expect(result).toEqual(mockDocuments);
    });

    it("throws error when companionId is missing", async () => {
      await expect(loadCompanionDocument("")).rejects.toThrow(
        "Companion ID missing"
      );
    });
    ...
    it("throws error when API call fails", async () => {
      ...
      await expect(loadCompanionDocument("comp-123")).rejects.toThrow(
        "API error"
      );
    });
  });
  ...
  describe("loadDocumentDetails", () => {
    ...
    it("throws error when documentId is missing", async () => {
      await expect(loadDocumentDetails("")).rejects.toThrow(
        "Document ID missing"
      );
    });
  ...
  describe("loadDocumentDownloadURL", () => {
    it("fetches download URL from the correct endpoint", async () => {
      const mockUrls = [{ url: "https://example.com/doc.pdf", key: "key-1", mimeType: "application/pdf" }];
    ...
    it("throws error when documentId is missing", async () => {
      await expect(loadDocumentDownloadURL("")).rejects.toThrow(
        "Document ID missing"
      );
    });

    it("throws error when documentId is undefined", async () => {
      await expect(loadDocumentDownloadURL(undefined)).rejects.toThrow(
        "Document ID missing"
      );
    });
    ...
    it("throws error when API call fails", async () => {
      ...
      await expect(loadDocumentDownloadURL("doc-123")).rejects.toThrow(
        "API error"
      );
    });
  });
