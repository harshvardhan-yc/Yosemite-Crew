...
describe('Service Service', () => {
  const mockServiceStoreSetServicesForOrg = jest.fn();
  ...
  beforeEach(() => {
    ...
    (useServiceStore.getState as jest.Mock).mockReturnValue({
      setServicesForOrg: mockServiceStoreSetServicesForOrg,
    });
  });
  ...
  describe('Validation', () => {
    it('returns empty array and warns if no orgId is provided or in store', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        'No primary organisation selected. Skipping service fetch.'
      );
  ...
  describe('Success Path', () => {
    it('fetches, transforms, and updates store on successful response', async () => {
      ...
      expect(mockServiceStoreSetServicesForOrg).toHaveBeenCalledWith('org-123', [
        mockTransformedService,
      ]);
  ...
  describe('Data Integrity', () => {
    it('returns empty array and warns if response data is not an array', async () => {
      ...
      expect(result).toEqual([]);
      expect(consoleSpy).toHaveBeenCalledWith('Services response is not an array.', {
        some: 'object, not array',
      });
      expect(mockServiceStoreSetServicesForOrg).not.toHaveBeenCalled();
      consoleSpy.mockRestore();
    });
  });
