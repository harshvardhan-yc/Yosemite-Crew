...
describe('Profile Service', () => {
  ...
  const mockProfileStoreGetProfileById = jest.fn();
  ...
  beforeEach(() => {
    ...
    (useUserProfileStore.getState as jest.Mock).mockReturnValue({
      ...
      getProfileById: mockProfileStoreGetProfileById,
    });
    ...
    mockedPutData.mockResolvedValue({ data: { profile: {} } });
  });
  ...
  describe('loadProfiles', () => {
    ...
    it('fetches profiles for multiple orgs and aggregates results', async () => {
      ...
      expect(mockProfileStoreSetProfiles).toHaveBeenCalledWith([
        ...
        expect.objectContaining({ id: 'prof-2' }),
      ]);
    });
    ...
    it('handles partial failures (Promise.allSettled) gracefully', async () => {
      ...
      mockedGetData.mockImplementation((url) => {
        if (url.includes('org-success'))
          return Promise.resolve({ data: { profile: { id: 'prof-ok' } } });
      ...
      expect(mockProfileStoreSetProfiles).toHaveBeenCalledWith([
        expect.objectContaining({ id: 'prof-ok' }),
      ]);
    ...
    it('catches and rethrows top-level errors (outside the loop)', async () => {
      ...
      mockProfileStoreSetProfiles.mockImplementation(() => {
        throw error;
      });
  ...
  describe('patchUserProfile', () => {
    it('patches profile and updates an existing profile in the store', async () => {
      const existingProfile = {
        _id: 'profile-1',
        organizationId: 'org-1',
        personalDetails: { phoneNumber: '123' },
      };
      ...
      mockedPutData.mockResolvedValue({
        data: {
          profile: {
            _id: 'profile-1',
            organizationId: 'org-1',
            personalDetails: { timezone: 'UTC+5:30 - Asia/Kolkata' },
          },
        },
      });

      await patchUserProfile('org-1', {
        personalDetails: { timezone: 'UTC+5:30 - Asia/Kolkata' },
      });

      expect(mockProfileStoreStartLoading).toHaveBeenCalled();
      expect(mockedPutData).toHaveBeenCalledWith('/fhir/v1/user-profile/org-1/profile', {
        _id: 'profile-1',
        organizationId: 'org-1',
        personalDetails: { phoneNumber: '123', timezone: 'UTC+5:30 - Asia/Kolkata' },
        professionalDetails: {},
      });
      ...
      expect(mockProfileStoreAddProfile).not.toHaveBeenCalled();
    });

    it('patches profile and adds to store when no existing profile exists', async () => {
      mockProfileStoreGetProfileById.mockReturnValue(undefined);
      mockedGetData.mockResolvedValue({
        data: {
          profile: {
            _id: 'profile-2',
            organizationId: 'org-2',
            personalDetails: { phoneNumber: '456' },
          },
        },
      });
      mockedPutData.mockResolvedValue({
        data: {
          profile: {
            _id: 'profile-2',
            organizationId: 'org-2',
            personalDetails: { timezone: 'UTC - UTC' },
          },
        },
      });

      await patchUserProfile('org-2', {
        personalDetails: { timezone: 'UTC - UTC' },
      });

      expect(mockedGetData).toHaveBeenCalledWith('/fhir/v1/user-profile/org-2/profile');
      expect(mockedPutData).toHaveBeenCalledWith('/fhir/v1/user-profile/org-2/profile', {
        _id: 'profile-2',
        organizationId: 'org-2',
        personalDetails: { phoneNumber: '456', timezone: 'UTC - UTC' },
        professionalDetails: {},
      });
      expect(mockProfileStoreUpdateProfile).toHaveBeenCalledWith({
        _id: 'profile-2',
    ...
    it('logs and rethrows errors', async () => {
      ...
      mockProfileStoreGetProfileById.mockReturnValue({
        _id: 'profile-1',
        organizationId: 'org-1',
      });
      ...
      await expect(
        patchUserProfile('org-1', {
          personalDetails: { timezone: 'UTC+5:30 - Asia/Kolkata' },
        })
      ).rejects.toThrow('Patch Failed');

      expect(consoleSpy).toHaveBeenCalledWith('Failed to patch profile:', error);
      consoleSpy.mockRestore();
    });
  });
  ...
  describe('createUserProfile', () => {
    ...
    it('sends POST request and updates store on success (no existing DRAFT)', async () => {
      ...
      expect(mockProfileStoreStartLoading).toHaveBeenCalled();
      expect(mockedPostData).toHaveBeenCalledWith('/fhir/v1/user-profile/org-1/profile', {
        ...mockInput,
        organizationId: 'org-1',
      });
  ...
  describe('upsertUserProfile', () => {
    ...
    it('sends PUT request using payload orgId and updates store', async () => {
      ...
      expect(mockProfileStoreStartLoading).toHaveBeenCalled();
      expect(mockedPutData).toHaveBeenCalledWith('/fhir/v1/user-profile/org-99/profile', mockInput);
      expect(mockProfileStoreUpdateProfile).toHaveBeenCalledWith(mockResponseData);
    });
