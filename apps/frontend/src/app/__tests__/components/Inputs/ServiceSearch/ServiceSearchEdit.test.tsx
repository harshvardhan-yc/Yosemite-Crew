...
jest.mock('@/app/lib/specialities', () => ({
  specialties: [{ name: 'General Medicine', services: ['Checkup', 'Consultation', 'Vaccination'] }],
  specialtiesByKey: {
    'General Medicine': {
...
describe('ServiceSearchEdit Component', () => {
  beforeEach(() => {
    ...
    (useOrgStore as unknown as jest.Mock).mockImplementation((selector) =>
      selector({
        orgsById: { [mockOrgId]: { _id: mockOrgId, type: 'HOSPITAL' } },
        primaryOrgId: mockOrgId,
      })
    );
  });
  ...
  it("shows 'Add service' button when no suggestions match", () => {
    ...
    expect(screen.queryByText('Checkup')).not.toBeInTheDocument();
    expect(screen.getByText('Add service “Custom Service”')).toBeInTheDocument();
  });
  ...
  it('does nothing if trying to add an empty service (defensive check)', async () => {
    ...
    const fullSpec = {
      ...mockSpeciality,
      services: [{ name: 'Checkup' }, { name: 'Consultation' }, { name: 'Vaccination' }] as any,
    };
