...
describe("OrgInvites Component", () => {
  ...
  it("handles successful invite acceptance (Table Button)", async () => {
    ...
    // Find accept buttons (rendered via icon mock in GenericTable mock)
    const acceptButtons = screen.getAllByTestId("icon-check");
    // Click the first one (Tech Corp)
  ...
  it("handles reject click without crashing (placeholder function)", () => {
    ...
    // Find reject buttons
    const rejectButtons = screen.getAllByTestId("icon-close");
    // Click first reject button
    fireEvent.click(rejectButtons[0].closest("button")!);

    // Since the function is empty in source: const handleReject = (invite: Invite) => {};
