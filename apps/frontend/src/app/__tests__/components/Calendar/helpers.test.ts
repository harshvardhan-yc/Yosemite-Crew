...
// FIX 1: Map 'start' and 'end' Dates to 'startTime' and 'endTime' properties
// to satisfy the failed logic: a.startTime.getTime()
const createMockAppt = (start: Date, end: Date): Appointment =>
  ({
    _id: "test-id",
    start,
    end,
    // Provide Date objects for properties accessed by sorting/layout logic:
    startTime: start, // <-- CHANGED to Date object
    endTime: end, // <-- CHANGED to Date object
   
    // Add other required fields to satisfy TS
...
// Helper to create a minimal valid task using TasksProps (no changes needed here)
