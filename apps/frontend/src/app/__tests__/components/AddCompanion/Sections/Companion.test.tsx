...
describe("Companion Component", () => {
  ...
  it("fetches companions when parentId exists (useEffect)", async () => {
    ...
    await act(async () => {
      render(
        <Companion
          {...defaultProps}
          parentFormData={{ ...EMPTY_STORED_PARENT, id: "p1" }}
        />,
      );
    });
  ...
  it("handles search dropdown selection", async () => {
    ...
    await waitFor(() =>
      expect(companionService.getCompanionForParent).toHaveBeenCalled(),
    );
  ...
  it("updates form data on input changes", async () => {
    ...
    expect(mockSetFormData).toHaveBeenCalledWith(
      expect.objectContaining({ gender: "new-value" }),
    );
    ...
    fireEvent.change(
      screen.getByTestId("input-Current weight (optional) (lbs)"),
      {
        target: { value: "20" },
      },
    );
    ...
    fireEvent.click(
      screen.getByTestId("dropdown-Country of origin (optional)"),
    );
    expect(mockSetFormData).toHaveBeenCalled();
  });
  ...
  it("toggles Neutered status", async () => {
    await act(async () => {
      render(
        <Companion
          {...defaultProps}
          formData={{ ...validFormData, isneutered: false }}
        />,
      );
    });
  ...
  it("toggles Insurance and shows extra fields", async () => {
    const { rerender } = render(
      <Companion
        {...defaultProps}
        formData={{ ...validFormData, isInsured: false }}
      />,
    );
  ...
  it("validates required fields on submit", async () => {
    ...
    expect(screen.getByTestId("error-dob")).toHaveTextContent(
