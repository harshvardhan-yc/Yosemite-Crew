...
const buildFallbackForm = (qr: any): { form: Form; submission: FormSubmission } => {
  ...
  const formId =
    ...
    qr.id ||
    `form-${Date.now()}`;
  const fallbackName = qr.title || firstItemText || 'Form submission';
...
const mapItem = (
  ...
  try {
    ...
    const isExplicitlyPending = rawStatus.includes('pending') || rawStatus.includes('incomplete');
    const isExplicitlyCompleted = rawStatus.includes('complete') || rawStatus.includes('signed');
...
export const fetchAppointmentForms = async (
  appointmentId: string
): Promise<AppointmentFormsResponse> => {
...
export const linkAppointmentForms = async (params: {
  ...
  await postData(`/fhir/v1/appointment/pms/${organisationId}/${appointmentId}/forms`, { formIds });
};
