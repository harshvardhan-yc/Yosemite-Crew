...
const PrescriptionFormSection = <K extends SoapKey>({
    ...
    try {
      ...
      setFormData((prev) => {
        ...
        if (afterResult?.lineItems?.length) {
          next.lineItems = [...afterResult.lineItems, ...(prev.lineItems ?? [])];
        }
        return next;
      });
      setActive(null);
  ...
  return (
    <PermissionGate allOf={[PERMISSIONS.PRESCRIPTION_VIEW_ANY]} fallback={<Fallback />}>
      <div className="flex flex-col gap-6 w-full flex-1 justify-between overflow-y-auto scrollbar-hidden">
        <Accordion title={title} defaultOpen={true} showEditIcon={false} isEditing={true}>
        ...
        {canEdit && active && <Primary href="#" text={actionText} onClick={handleAction} />}
      </div>
    </PermissionGate>
  );
};
