...
import { CompanionProps } from "../../DataTable/types";
    ...
    <div className="w-full flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {Species.map((specie) => (
            <button
              ...
              className={`min-w-20 h-9 rounded-xl! border font-satoshi! text-[15px]! font-bold ${specie.key === activeSpecie ? "border-blue-text! bg-blue-light! text-blue-text!" : "border-[#302f2e]!"}`}
            >
              {specie.name}
            </button>
          ))}
      ...
      <div className="flex items-center gap-2">
        {Statuses.map((status) => (
          <button
            key={status.key}
            className={`min-w-20 h-9 rounded-xl! font-satoshi! text-[15px]! font-bold hover:shadow-[0_0_8px_0_rgba(0,0,0,0.16)] ${status.key === activeStatus ? "border!" : "border-0!"}`}
            style={{
